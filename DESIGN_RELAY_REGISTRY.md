...
| S2 | Forwarding semantics | ~~Decide v1 policy…~~ **Done (2026-07 dual-envelope).** `RemoteSearchRequest` v2: requester sig = `queryCanonicalBytes()` only (no ttl/path). `withNextHop` preserves sig. `MULTI_HOP_FORWARDING_ENABLED=true`; performer `ttl=DEFAULT_TTL` (2). Forwarder-first mesh: `PeerRegistrySync` registers self + imports `/lookup` peers into `PeerDirectory`. | Regression: `dualEnvelopeHopPreservesRequesterVerify`, `multiHopForwardsPreservingRequesterSignature`. | `[common] Dual-envelope multi-hop + mesh peer import` |
...
| S9 | Content-aware forwarding | **Done (2026-09).** Added `IndexDigest` (bounded keyword Bloom, Unicode-normalized, no false negatives) and `MeshProtocolId.INDEX_DIGEST=8`. `PeerRegistrySync` announces the local index digest to directory peers; `IncomingSearchRequestHandler`/`IceBridgeSearchTransport` accept and store it in `PeerDirectory`; `IncomingSearchRequestHandler.forwardRequest` and `DistributedSearchPerformer.selectPeers` select digest matches first, filling remaining fan-out with live peers. Replaces blind random fan-out. | `IndexDigestTest` (no false negatives, normalization, bounds, FP rate), `PeerDirectoryRoutingTest.holderWithMatchingDigestIsSelectedFirst`, `IceBridgeSearchTransportTest.indexDigestFramesAreDeliveredAndStoredForHolderRouting`. | `[common] Route search to index holders via announced digest` |
| S10 | Peer liveness pruning | **Done (2026-09).** `PeerDirectory` tracks `lastContactMs`/`failures`; any authenticated inbound frame or verified response calls `markContact`, failed sends call `markFailure` (drop at `MAX_FAILURES`); `evictUnreachable` prunes silent/unaffirmed peers from the transport maintenance timer; `PeerRegistrySync` skips stale `/lookup` rows (`MAX_LOOKUP_AGE_MS`). Unreachable peers are removed instead of retried forever. | `PeerDirectoryRoutingTest` (eviction, contact reset, sweep, not sampled), `IceBridgeSearchTransportTest` liveness assertion. | `[common] Prune unreachable peers and gate selection on liveness` |
...
3. It sends dual-envelope signed requests with `ttl=DEFAULT_TTL` (2); hop fields may change on multi-hop without re-signing.
...
- ~~**Dual-envelope multi-hop**~~ **Done (2026-07):** requester signs immutable query envelope only; hop path/ttl mutable without re-sign; multi-hop enabled with hop limit 2. Optional future: separate hopSig chain for hop authenticity audits.
