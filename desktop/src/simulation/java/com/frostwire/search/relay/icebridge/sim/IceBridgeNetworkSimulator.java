 ...
 *
 * <ul>
 *   <li><b>EC2 IceBridge hubs</b> — high-bandwidth forwarders we operate (fat backbone; high fanout
 *       is affordable)
 *   <li><b>Optional edge IceBridges</b> — lower-capacity community/home relays
 *   <li><b>FrostWire clients</b> — leaves multi-homed to hubs (prefer EC2)
 * </ul>
 *
 * <p>Unlike residential LimeWire ultrapeers, EC2 hubs share a high-speed mesh, so N (mesh fanout)
 * is constrained more by amplification policy than by uplink. M (search peer fanout) is the main
 * last-hop cost.
 *
 * <p>Not packet-level rUDP — topology + fanout/TTL economics for auto-research.
 *
 * <p><b>Model limitation:</b> on small hub meshes a flood visits nearly every IceBridge regardless
 * of N (mesh saturation), so N then discriminates only traffic cost, not coverage. Recall-sensitive
 * conclusions about N require the larger study network ({@code runHybridEc2StudyResearch}), where
 * the mesh is sparse enough for fanout to gate coverage.
 */
public final class IceBridgeNetworkSimulator {
  ...
  public static final class Config {
    ...
    /**
     * Prefer attaching leaves to EC2 hubs this fraction of the time (hybrid). 1.0 = always prefer
     * hubs when available.
     */
  ...
  public static final class SearchResult {
    ...
    SearchResult(
        ...
        boolean rareQuery,
        int meshMessages,
        int searchPeerMessages,
        int iceBridgesVisited,
        int frostWiresQueried,
        int contentHoldersReached,
        int hopsToFirstHit) {
      this.hit = hit;
  ...
  public static final class AggregateReport {
    ...
    AggregateReport(
        ...
        int iceBridgeEdges,
        int ec2Hubs,
        double meanMeshMessages,
        double meanSearchPeerMessages,
        double meanIceBridgesVisited,
        double meanFrostWiresQueried,
        double hitRate,
        double rareHitRate,
        double commonHitRate,
        double p95MeshMessages,
        double p95SearchPeerMessages,
        double meanHopsToHit,
        long worstCaseInfiniteMesh,
        long worstCaseInfiniteSearch,
        double score) {
      this.config = config;
    ...
    public String toString() {
      return String.format(
          ...
          config.profile,
          config.iceBridgeCount,
          ec2Hubs,
          config.frostWireCount,
          iceBridgeEdges,
          config.searchTrials,
          hitRate * 100,
          rareHitRate * 100,
          ...
          meanMeshMessages,
          p95MeshMessages,
          meanSearchPeerMessages,
          p95SearchPeerMessages,
          meanIceBridgesVisited,
          meanFrostWiresQueried,
          meanHopsToHit,
          config.meshFanout,
          config.searchPeerFanout,
          config.meshHopTtl,
          config.searchTtl,
          config.softMax,
          config.leafUpConnections);
    }
  }

  /**
   * SLO-style budgets for auto-research. EC2 hubs tolerate higher mesh traffic; last-hop leaf
   * queries are the scarcer resource.
   */
  ...
  private void buildNetwork() {
    ...
    int hubs =
        config.profile == Profile.HYBRID_EC2 ? Math.min(Math.max(2, config.ec2HubCount), ib) : ib;
    for (int i = 0; i < ib; i++) {
  ...
  public SearchResult runSearch() {
    ...
    while (!q.isEmpty()) {
      ...
      if (!leaves.isEmpty()) {
        ...
        int mCap =
            isEc2Hub[ib] ? config.searchPeerFanout : Math.max(3, config.searchPeerFanout / 2);
    ...
    if (config.searchTtl > 1 && !fwQueried.isEmpty()) {
      ...
      for (int depth = 1; depth < config.searchTtl; depth++) {
        ...
        int hopBudget =
            Math.max(
                config.searchPeerFanout, config.searchPeerFanout * Math.min(8, frontier.size()));
        int hopUsed = 0;
  ...
  /**
   * Multi-objective score in [roughly -1, 1]: high hit rates, low traffic. Candidates that miss
   * SLOs are heavily penalized.
   */
  public static double score(
    ...
    double p95Norm =
        0.5 * Math.min(1.0, p95Mesh / Math.max(1.0, b.maxP95MeshMessages))
