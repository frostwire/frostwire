...
public class TelluridePlaylistTests {
  ...
  public void testIsYouTubePlaylistUrl_ChannelId() {
    assertTrue(
        SearchMediator.isYouTubePlaylistUrl(
            "https://www.youtube.com/channel/UCYD4cCGX7OjeHY5PUBwwkLg"));
  }
  ...
  public void testIsYouTubePlaylistUrl_PlaylistListParam() {
    assertTrue(
        SearchMediator.isYouTubePlaylistUrl(
            "https://www.youtube.com/playlist?list=PLrAXtmErZgOeiKm4sgNOknGvNj3_w7fOc"));
  }
  ...
  public void testIsYouTubePlaylistUrl_WatchWithList() {
    assertFalse(
        SearchMediator.isYouTubePlaylistUrl(
            "https://www.youtube.com/watch?v=abc123&list=PLrAXtmErZgOeiKm4sgNOknGvNj3_w7fOc"));
  }
  ...
  public void testIsYouTubePlaylistUrl_MusicPlaylist() {
    assertTrue(
        SearchMediator.isYouTubePlaylistUrl(
            "https://music.youtube.com/playlist?list=RDCLAK5oy_kuACt4MGKsQNN3VEHORXa5e9TPNz2C5CI"));
  }
  ...
  public void testIsYouTubePlaylistUrl_MobileChannelId() {
    assertTrue(
        SearchMediator.isYouTubePlaylistUrl(
            "https://m.youtube.com/channel/UCYD4cCGX7OjeHY5PUBwwkLg"));
  }
  ...
  public void testGetValidPlaylistResults_BasicPlaylist() {
    ...
    List<TellurideSearchResult> results =
        TellurideSearchPerformer.getValidPlaylistResults(json, gson, null, -1, "test");
    ...
    assertTrue(
        "Cloud:youtube".equals(r1.getSource()) || "Cloud:YoutubeTab".equals(r1.getSource()),
        "source was: " + r1.getSource());
  ...
  public void testGetValidPlaylistResults_NullEntries() {
    String json =
        "{\"type\":\"playlist\",\"title\":\"Empty\",\"extractor\":\"youtube\",\"entries\":null}";
    List<TellurideSearchResult> results =
        TellurideSearchPerformer.getValidPlaylistResults(json, gson, null, -1, "test");
  ...
  public void testGetValidPlaylistResults_EmptyEntries() {
    String json =
        "{\"type\":\"playlist\",\"title\":\"Empty\",\"extractor\":\"youtube\",\"entries\":[]}";
    List<TellurideSearchResult> results =
        TellurideSearchPerformer.getValidPlaylistResults(json, gson, null, -1, "test");
    assertTrue(results.isEmpty());
  }
  ...
  public void testGetValidPlaylistResults_MissingUploadDate() {
    ...
    assertEquals(1, results.size());
    assertTrue(
        results.get(0).getCreationTime() > 0,
        "creationTime should default to current time when upload_date is null");
  }
  ...
  public void testPartialConstructor6Params() {
    ...
    assertTrue(
        "Cloud:youtube".equals(result.getSource()) || "Cloud:YoutubeTab".equals(result.getSource()),
        "source was: " + result.getSource());
  ...
  public void testPlaylistModeIntegration() throws InterruptedException {
    ...
    if (tellurideLauncherFile == null
        || !tellurideLauncherFile.exists()
        || !tellurideLauncherFile.canExecute()) {
    ...
    TellurideListener listener =
        new TellurideListener() {
          @Override
          ...
          public void onMeta(String json) {
            LOG.info(
                "[testPlaylistModeIntegration] GOT JSON: "
            ...
            if (json.contains("\"type\"") && json.contains("\"playlist\"")) {
              ...
              errors.add(
                  "Expected playlist JSON but got: "
    ...
    TellurideLauncher.launch(
        tellurideLauncherFile,
    ...
    if (!completed) {
      LOG.warn(
          "[testPlaylistModeIntegration] Timed out - telluride binary likely needs rebuild with -p flag support");
      return;
    }
    if (downloadStarted[0]) {
      LOG.warn(
          "[testPlaylistModeIntegration] Telluride started downloading instead of extracting playlist - binary needs rebuild with -p flag");
      return;
    }
    if (!errors.isEmpty()) {
      LOG.warn(
          "[testPlaylistModeIntegration] Skipping assertions due to errors: "
              + String.join("; ", errors));
      return;
    }
    ...
    assertTrue(
        "Cloud:youtube".equals(firstResult.getSource())
            || "Cloud:YoutubeTab".equals(firstResult.getSource()),
        "source was: " + firstResult.getSource());
