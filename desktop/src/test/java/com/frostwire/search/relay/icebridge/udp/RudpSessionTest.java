...
class RudpSessionTest {
  ...
  void receiveRemoteAcceptsInOrderSequence() {
    RudpSession session =
        new RudpSession(1, 2, new InetSocketAddress("127.0.0.1", 6000), null, false);
  ...
  void receiveRemoteWrapsFromMaxToIntMin() {
    ...
    assertTrue(
        Integer.compareUnsigned(next, current) > 0,
  ...
  void ackLocalUsesUnsignedComparison() {
    RudpSession session =
        new RudpSession(1, 2, new InetSocketAddress("127.0.0.1", 6000), null, true);
    ...
    session.markSent(Integer.MIN_VALUE);
    assertTrue(session.ackLocal(Integer.MIN_VALUE));
    assertEquals(
        0, session.pending().size(), "pending should be cleared when ack wraps past MAX_VALUE");
  }
  ...
  void ackLocalRejectsOldAckUnderUnsigned() {
    ...
    assertTrue(session.ackLocal(100));
  ...
  void remotePubClonedDefensively() {
    ...
    pub[0] = 42;
    RudpSession session =
        new RudpSession(1, 2, new InetSocketAddress("127.0.0.1", 6000), pub, false);
  ...
  @Test
  void fullUnsignedSequenceSpaceNeverWrapsIntoHandshakeZero() throws Exception {
    RudpSession session =
        new RudpSession(1, 1, new InetSocketAddress("127.0.0.1", 6000), null, true);
    java.lang.reflect.Field field = RudpSession.class.getDeclaredField("nextLocalSeq");
    field.setAccessible(true);
    ((java.util.concurrent.atomic.AtomicInteger) field.get(session)).set(-1);
    assertTrue(session.hasSequenceCapacity(1));
    assertFalse(session.hasSequenceCapacity(2));
    assertEquals(-1, session.nextLocalSequence());
    assertFalse(session.hasSequenceCapacity(1));
    assertThrows(IllegalStateException.class, session::nextLocalSequence);
  }
}
