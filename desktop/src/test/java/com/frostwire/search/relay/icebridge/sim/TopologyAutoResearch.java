...
/**
 * Auto-research loop over IceBridge topology parameters for a fixed
 * network profile (hybrid EC2 hubs by default).
 *
 * <p>Objective: maximize {@link IceBridgeNetworkSimulator#score} subject to
 * hit-rate and traffic SLOs — so users find content while the EC2 backbone
 * and last-hop leaf queries stay within budget.
 *
 * <p>Produces a ranked list of candidates; the winner can be applied via
 * {@link IceBridgeTopology#applyRemote}.
 */
public final class TopologyAutoResearch {
    ...
    public static final class Candidate {
        ...
        Candidate(int n, int m, int meshTtl, int searchTtl, int softMax, int leafUps,
                  AggregateReport report, boolean meetsSlos) {
            this.meshFanout = n;
        ...
        public String toString() {
            return String.format(
                    ...
                    meshFanout, searchPeerFanout, meshHopTtl, searchTtl, softMax, leafUpConnections,
                    report.score, meetsSlos, report.hitRate * 100, report.rareHitRate * 100,
                    report.meanMeshMessages, report.meanSearchPeerMessages);
        }
    }
    ...
    public static final class Result {
        ...
        Result(Candidate best, Candidate bestMeetingSlos, List<Candidate> ranked,
               Config networkTemplate, ResearchBudgets budgets) {
            this.best = best;
    ...
    /**
     * Hybrid EC2 benchmark network: fat hub mesh we operate + many leaves.
     * Sized for unit-test auto-research (seconds). Scale up for offline studies.
     */
    ...
    /**
     * Budgets for high-speed EC2 backbone: mesh can be heavier; last-hop
     * leaf query volume is the main limit.
     */
    ...
    public static Result run(Config networkTemplate, ResearchBudgets budgets, Grid grid) {
                ...
                for (int meshTtl : grid.meshHopTtls) {
                    for (int searchTtl : grid.searchTtls) {
                        for (int softMax : grid.softMaxes) {
                            // softMax must be >= max of TTLs used as start remaining
                            ...
                            for (int leafUps : grid.leafUps) {
                                ...
                                // Stable seed per candidate for fair compare
                                ...
                                all.add(new Candidate(n, m, meshTtl, searchTtl, softMax, leafUps,
                                        report, slo));
                                evaluated++;
                            }
                        }
        ...
        LOG.info("TopologyAutoResearch evaluated " + evaluated + " candidates; "
                + "best=" + best + "; bestSLO=" + bestSlo);
        if (best != null) {
    ...
    /**
     * Apply the research winner to the process-wide topology (if any).
     *
     * @return true if a SLO-meeting candidate was applied
     */
    public static boolean applyWinner(Result result) {
        Candidate w = result.bestMeetingSlos != null ? result.bestMeetingSlos : result.best;
        if (w == null) {
            return false;
        }
        IceBridgeTopology.get().applyRemote(
                w.meshFanout, w.searchPeerFanout, w.meshHopTtl, w.searchTtl,
                w.softMax, w.leafUpConnections);
        return result.bestMeetingSlos != null;
    }
}
