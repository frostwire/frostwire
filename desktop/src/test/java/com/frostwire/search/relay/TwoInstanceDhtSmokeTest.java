...
/**
 * Headless end-to-end smoke test of two FrostWire relay nodes wired through a real (in-process)
 * DHT.
 *
 * <p>Uses {@link LocalDhtCluster} for the SessionManager — each relay node has a real DHT-capable
 * jlibtorrent session. The cluster's DHT is on a local-only bootstrap which is not a full BEP 5
 * routing network, so we don't assert DHT-mediated discovery round-trips. We DO assert:
 *
 * <ul>
 *   <li>The relay protocol between two nodes with distinct identities works end-to-end (publish /
 *       find / unpublish / not-find) — see {@link MultiInstanceRelayTest} for the canonical case
 *       and this class for the SessionManager-backed case.
 *   <li>The DHT advertiser runs without error and exercises the BEP 5 / BEP 46 code paths against
 *       the real SessionManager.
 *   <li>The DHT discovery runs without error and the source/fetcher code paths are exercised.
 * </ul>
 *
 * <p>For the actual two-instance DHT round-trip (advertise → discover → query), a real DHT network
 * is required. The {@link LocalDhtCluster} is suitable for DHT primitive testing but its
 * localhost-only bootstrap doesn't form a proper BEP 5 routing network. The eventual verification
 * of the cross-machine scenario is a manual smoke test, since it requires two real FrostWire
 * desktop instances connecting through the public DHT.
 */
...
class TwoInstanceDhtSmokeTest {
  ...
  void twoNodesDoFullPublishDiscoverRelayQueryFlow() throws Exception {
    ...
    Optional<RemoteSearchResponse> resp = nodeB.queryNodeA(nodeA, "alpha");
    ...
    nodeA.advertise();
    nodeB.discover();
    Optional<RemoteSearchResponse> respAfter = nodeB.queryNodeA(nodeA, "alpha");
  ...
  void nodeAAdvertiseRunsWithoutErrorOnRealSession() {
    ...
    assertTrue(
        nodeA.identityPublisher.lastPublishEpochSec() > 0,
  ...
  /**
   * A self-contained relay node backed by a real DHT SessionManager from the {@link
   * LocalDhtCluster}.
   */
  private static final class DhtRelayNode implements AutoCloseable {
    ...
    DhtRelayNode(SessionManager session, String label) throws Exception {
      ...
      this.index =
          LocalIndexTable.open(
              new java.io.File(
                  System.getProperty("java.io.tmpdir"),
                  "dht-relay-smoke-" + label + "-" + System.nanoTime() + ".db"));
