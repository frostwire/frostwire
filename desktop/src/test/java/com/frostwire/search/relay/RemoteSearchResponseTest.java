...
class RemoteSearchResponseTest {
  ...
  void builderAcceptsMinimalValid() {
    RemoteSearchResponse r =
        RemoteSearchResponse.builder().nonce(new byte[32]).signature(new byte[64]).build();
    assertNotNull(r);
  ...
  @Test
  void rowCtorCarriesPublicCatalogDirectly() {
    RemoteSearchResponse.Row r =
        new RemoteSearchResponse.Row(
            new byte[20],
            "n",
            1,
            1,
            ...
            true);
    assertTrue(r.publicCatalog, "direct Row ctor must carry the flag");
  }

  @Test
  void publicCatalogFlagRoundTripsOnV4() {
    byte[] nonce = new byte[32];
    byte[] infoHash = new byte[20];
    byte[] pub = new byte[32];
    RemoteSearchResponse v4 =
        RemoteSearchResponse.builder()
            .version(RemoteSearchResponse.VERSION_4)
            .nonce(nonce)
            .addRow(
                infoHash,
                "ubuntu",
            ...
            .build();
    assertTrue(
        v4.rows().get(0).publicCatalog,
        "builder row flag=" + v4.rows().get(0).publicCatalog + " version=" + v4.version());

    RemoteSearchResponse decoded = RemoteSearchResponse.fromBencodeableMap(v4.toBencodeableMap());
    assertNotNull(decoded);
    assertEquals(RemoteSearchResponse.VERSION_4, decoded.version());
    assertTrue(decoded.rows().get(0).publicCatalog, "pc must survive the wire");
    assertArrayEquals(
        v4.canonicalBytes(),
        decoded.canonicalBytes(),
        "decode must reproduce the signed row hash, including pc");
  }

  @Test
  void publicCatalogChangesTheSignatureDomain() {
    byte[] nonce = new byte[32];
    byte[] infoHash = new byte[20];
    byte[] pub = new byte[32];
    RemoteSearchResponse on =
        RemoteSearchResponse.builder()
            .version(RemoteSearchResponse.VERSION_4)
            .nonce(nonce)
            .addRow(
    ...
    RemoteSearchResponse off =
        RemoteSearchResponse.builder()
            ...
            .signature(new byte[64])
            .build();
    assertFalse(java.util.Arrays.equals(on.canonicalBytes(), off.canonicalBytes()));
  }

  @Test
  void v3ResponsesNeverCarryPublicCatalog() {
    byte[] nonce = new byte[32];
    byte[] infoHash = new byte[20];
    byte[] pub = new byte[32];
    RemoteSearchResponse v3 =
        RemoteSearchResponse.builder()
            .version(RemoteSearchResponse.VERSION_3)
            .nonce(nonce)
            .addRow(
                infoHash,
                "ubuntu",
            ...
            .signature(new byte[64])
            .build();
    for (Object row : (java.util.List<?>) v3.toBencodeableMap().get("rows")) {
      assertFalse(((java.util.Map<?, ?>) row).containsKey("pc"), "v3 rows must not emit pc");
    }
    RemoteSearchResponse decoded = RemoteSearchResponse.fromBencodeableMap(v3.toBencodeableMap());
    assertNotNull(decoded);
    assertFalse(decoded.rows().get(0).publicCatalog, "v3 decode defaults to not crawlable");
  }
  ...
  void canonicalBytesDifferWhenMatchedFileChanges() {
    ...
    byte[] infoHash = new byte[20];
    byte[] pub = new byte[32];
    RemoteSearchResponse withoutMf =
        RemoteSearchResponse.builder()
            .nonce(nonce)
            .timestamp(ts)
    ...
    RemoteSearchResponse withMf =
        RemoteSearchResponse.builder()
            ...
            .build();
    assertFalse(
        java.util.Arrays.equals(withoutMf.canonicalBytes(), withMf.canonicalBytes()),
  ...
  void canonicalBytesDifferOnMatchedFileContentChange() {
    ...
    RemoteSearchResponse b =
        RemoteSearchResponse.builder()
            ...
            .build();
    assertFalse(
        java.util.Arrays.equals(a.canonicalBytes(), b.canonicalBytes()),
  ...
  void matchedFileSurvivesBencodeableMapRoundTrip() {
    ...
    assertEquals("docs/readme.txt", firstRow.get("mf"), "toBencodeableMap must include 'mf' field");

    RemoteSearchResponse decoded = RemoteSearchResponse.fromBencodeableMap(map);
    assertNotNull(decoded);
    assertEquals(
        "docs/readme.txt",
        decoded.rows().get(0).matchedFile,
  ...
  void signatureWithMatchedFileVerifiesCorrectly() throws Exception {
    ...
    assertTrue(verifier.verify(signature), "Signature over response with matchedFile must verify");

    // Tamper with matchedFile — signature must fail
    RemoteSearchResponse tampered =
        RemoteSearchResponse.builder()
            .nonce(unsigned.nonce())
            .timestamp(unsigned.timestamp())
