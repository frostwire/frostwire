...
class PeerRegistryTest {
  ...
  void registerRefreshesExistingPeer() {
    ...
    assertTrue(
        registry.register(
            new PeerRecord(pub, "1.2.3.4", 6888, IceBridgeConfig.Role.FORWARDER, 1000)));
    assertTrue(
        registry.register(
            new PeerRecord(pub, "5.6.7.8", 6889, IceBridgeConfig.Role.FORWARDER, 2000)));
  ...
  @Test
  void touchRefreshesKnownPeerLivenessWithoutChangingRoleOrVersion() {
    PeerRegistry registry = new RegistryBuilder().build();
    byte[] pub = new byte[32];
    pub[0] = 7;
    ...
    assertTrue(
        registry.register(
            new PeerRecord(pub, "1.2.3.4", 6888, IceBridgeConfig.Role.BOTH, 1000, "1.1.0")));
    PeerRecord before = registry.lookup(pub);
    assertNotNull(before);
    assertEquals(1000, before.lastSeenMs());

    registry.touch(pub);
    PeerRecord after = registry.lookup(pub);
    assertNotNull(after);
    assertTrue(after.lastSeenMs() > before.lastSeenMs(), "touch must refresh lastSeen");
    assertEquals(IceBridgeConfig.Role.BOTH, after.role(), "role must be preserved");
    assertEquals("1.1.0", after.icebridgeVersion(), "version must be preserved");

    byte[] unknown = new byte[32];
    unknown[0] = 9;
    registry.touch(unknown);
    assertNull(registry.lookup(unknown), "touch must not create entries");
  }
  ...
  void lookupForwardersOnlyReturnsForwarders() {
    ...
    registry.register(
        new PeerRecord(forwarder, "1.2.3.4", 6888, IceBridgeConfig.Role.FORWARDER, 1000));
  ...
  void rateLimitRejectsSpam() {
    ...
    PeerRecord record =
        new PeerRecord(
            pub, "1.2.3.4", 6888, IceBridgeConfig.Role.FORWARDER, System.currentTimeMillis());
  ...
  private static PeerRegistry registryWithForwarders(String prefix, int from, int count) {
    ...
    for (int i = from; i < from + count; i++) {
      ...
      registry.register(
          new PeerRecord(pub, (10 + i) + ".0.0.1", 6888, IceBridgeConfig.Role.FORWARDER, now));
    }
    return registry;
  }
