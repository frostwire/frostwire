...
class PeerDiscoveryTest {
  ...
  void constructorRejectsNullArgs() {
    assertThrows(IllegalArgumentException.class, () -> new PeerDiscovery(null, directory));
    assertThrows(IllegalArgumentException.class, () -> new PeerDiscovery(source, null));
  }
  ...
  void fetchIdentityRecordReturnsParsedRecord() throws Exception {
    KeyPair kp = java.security.KeyPairGenerator.getInstance("Ed25519").generateKeyPair();
  ...
  void placeholderDiscoveryStillWorksWithoutAuthenticator() {
    ...
    assertEquals(1, result.size());
    assertEquals(1, directory.size());
    PeerDirectory.PeerInfo info =
        directory.get(PeerDiscovery.placeholderPubkey("10.0.0.1", 6888)).orElse(null);
    assertNotNull(info);
    assertFalse(info.isVerified());
  }

  @Test
  void fetchEndpointsAlwaysIncludesRelaysForAggressiveDiscovery() {
    // Simulate source returning peers and relays (as DhtPeerDiscoverySource does)
    // We expect both to be considered (relays first in current impl) so relayers are found easily.
    source.endpoints.add(new DiscoveredEndpoint("10.0.0.1", 6889)); // peer
    // Relays would be added by the source impl; here we just ensure no filtering hides them.
    // The key regression test: discovery does not drop relay candidates.
    List<DiscoveredEndpoint> result = discovery.discoverAndRegister();
    // Without authenticator we get placeholders; main point is endpoints are processed.
