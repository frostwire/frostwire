...
/**
 * End-to-end integration test for the relay stack: spin up two or more self-contained relay nodes,
 * each on its own TCP port and with its own Ed25519 identity and SQLite-backed local index.
 *
 * <p>Verifies the canonical publish / find / unpublish / not-find flow that is the core value of
 * the distributed relay search:
 *
 * <ol>
 *   <li>Node A inserts a row into its local index.
 *   <li>Node B (via {@link OutgoingRelayClient}) dials Node A's relay port, sends a signed {@link
 *       RemoteSearchRequest}, and gets the row back.
 *   <li>Node A removes the row from its index.
 *   <li>Node B queries again and gets nothing.
 * </ol>
 *
 * <p>Additional tests exercise the trust-graph: spam marking and positive trust-floor rejection.
 *
 * <p>This test does NOT go through the DHT (which would need {@code LocalDhtCluster}). It uses
 * {@link OutgoingRelayClient} directly against the other node's {@link IncomingRelayServer}, which
 * is the protocol-level integration.
 */
class MultiInstanceRelayTest {
  ...
  void publishedTorrentIsDiscoverableThenUnpublishedRemovesIt() throws Exception {
    ...
    assertEquals(0, respAfter.get().rows().size(), "After unpublish, Node B should find nothing");
  }
  ...
  void nodeBCanQueryNodeCWhileNodeAIsolated() throws Exception {
    ...
    assertEquals(
        1,
        nodeA.search(nodeB, "second").get().rows().size(),
  ...
  void spamMarkedRequesterIsRejected() throws Exception {
    ...
    assertTrue(resp.isEmpty(), "Spam-marked requester must be rejected before service dispatch");
  }
  ...
  void unSpammingRestoresAccess() throws Exception {
    ...
    // Route refresh preserves local reputation; explicit eviction resets it.
  ...
  void unknownRequesterIsAcceptedByDefault() throws Exception {
    ...
    assertTrue(resp.isPresent(), "Unknown requester with default floor=0 should be accepted");
    assertEquals(1, resp.get().rows().size());
  }
  ...
  void positiveTrustFloorRejectsUnknownRequester() throws Exception {
    ...
    assertTrue(resp.isEmpty(), "Unknown requester should be rejected when target floor > 0");
  }

  // --- Helpers ---

  /**
   * Self-contained relay node: home dir, identity, local index, directory, role, server. Each
   * instance binds to a different ephemeral TCP port.
   */
  private static final class RelayNode implements AutoCloseable {
    ...
    RelayNode(File homeDir, int trustFloor, KeyPairBag requester) throws Exception {
      ...
      this.identity = IdentityKeys.generate(0);
      this.index = LocalIndexTable.open(new File(homeDir, LocalIndexTable.DEFAULT_DB_NAME));
      this.directory = new PeerDirectory(new NoopKarmaCache());
      this.service = new RelaySearchService(index, identity, hash -> index.get(hash).isPresent());
      this.role = new RelayRole(service, directory, trustFloor);
      IdentityRecord record =
          IdentityRecord.createSigned(
              identity.nodeId(), identity.ed25519(), identity.x25519PubRaw(), 0);
      this.server =
          new IncomingRelayServer(role, record, identity.ed25519().getPrivate(), 0, "127.0.0.1");
      this.server.start();
    ...
    Optional<RemoteSearchResponse> search(RelayNode target, String keywords) throws Exception {
      return searchAs(target, keywords, makeRequester());
    }

    Optional<RemoteSearchResponse> searchAs(RelayNode target, String keywords, KeyPairBag requester)
        throws Exception {
