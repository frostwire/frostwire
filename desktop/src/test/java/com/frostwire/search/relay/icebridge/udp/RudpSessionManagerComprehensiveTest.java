 ...
 *
 * <ul>
 *   <li>HELLO authentication and session creation
 *   <li>DATA delivery and ACK clearing
 *   <li>Fragmentation and reassembly of large payloads
 *   <li>Duplicate packet handling
 *   <li>Out-of-order packet rejection (v1 in-order requirement)
 *   <li>Concurrent sessions on a single manager
 *   <li>Retransmission with two real UDP servers
 *   <li>RELAY forwarding through a forwarder peer
 *   <li>RELAY_RESPONSE delivery to the listener
 *   <li>Metric counters
 * </ul>
 */
class RudpSessionManagerComprehensiveTest {

  private static final IceBridgeConfig CONFIG =
  ...
  void helloCreatesSessionAndDeliversData() throws Exception {
    ...
    RudpSessionManager mgr =
        new RudpSessionManager(remote, registry, metrics, (pub, payload) -> received.add(payload));
  ...
  void badHelloIsDropped() {
    RudpSessionManager mgr =
        new RudpSessionManager(remote, registry, metrics, (pub, payload) -> {});
    mgr.onPacket(
        new RudpPacketEnvelope(
  ...
  void ackForRealSessionIsAccepted() throws Exception {
    ...
    mgr.onPacket(
        new RudpPacketEnvelope(
            ...
            remoteAddr,
            new InetSocketAddress("127.0.0.1", 62004)));
    assertEquals(1, mgr.sessionCount());
  ...
  void duplicateDataIsIgnoredButReAcked() throws Exception {
    ...
    mgr.onPacket(
        new RudpPacketEnvelope(
            new RudpPacket(RudpPacket.Type.HELLO, cid, 0, 0, helloPayload),
            sender,
            new InetSocketAddress("127.0.0.1", 62011)));
  ...
  void outOfOrderDataIsRejected() throws Exception {
    ...
    mgr.onPacket(
        new RudpPacketEnvelope(
            new RudpPacket(RudpPacket.Type.HELLO, cid, 0, 0, helloPayload),
            sender,
            new InetSocketAddress("127.0.0.1", 62021)));
  ...
  void multipleConcurrentSessions() throws Exception {
    ...
    IdentityKeys[] peers = {
      IdentityKeys.generate(0), IdentityKeys.generate(0), IdentityKeys.generate(0)
    };
    ...
    for (int i = 0; i < 3; i++) {
      ...
      mgr.onPacket(
          new RudpPacketEnvelope(
              ...
              addrs[i],
              new InetSocketAddress("127.0.0.1", 63000)));
    }
  ...
  void largePayloadFragmentedAndReassembled() throws Exception {
    ...
    mgr.onPacket(
        new RudpPacketEnvelope(
            new RudpPacket(RudpPacket.Type.HELLO, cid, 0, 0, helloPayload),
            sender,
            new InetSocketAddress("127.0.0.1", 62031)));
  ...
  void payloadExactlyAtFragmentLimitUsesSingleData() throws Exception {
    ...
    mgr.onPacket(
        new RudpPacketEnvelope(
            new RudpPacket(RudpPacket.Type.HELLO, cid, 0, 0, helloPayload),
            sender,
            new InetSocketAddress("127.0.0.1", 62041)));
  ...
  void twoRealServersExchangeData() throws Exception {
    ...
    RudpSessionManager mgrA =
        new RudpSessionManager(local, registryA, metricsA, (pub, payload) -> {});
    RudpSessionManager mgrB =
        new RudpSessionManager(
            remote, registryB, metricsB, (pub, payload) -> receivedB.incrementAndGet());

    IceBridgeConfig configA =
        IceBridgeConfig.newBuilder()
            .rudpPort(freePort())
            .controlHttpPort(8797)
            .role(IceBridgeConfig.Role.BOTH)
            .build();
    IceBridgeConfig configB =
        IceBridgeConfig.newBuilder()
            .rudpPort(freePort())
            .controlHttpPort(8797)
            .role(IceBridgeConfig.Role.BOTH)
  ...
  void twoRealServersExchangeLargeFragmentedPayload() throws Exception {
    ...
    try {
      ...
      assertTrue(
          latch.await(15, TimeUnit.SECONDS), "large payload should be delivered and reassembled");
      assertArrayEquals(largePayload, receivedB.get(0), "reassembled payload must match original");
    } finally {
      serverA.close();
      serverB.close();
    }
  ...
  void relayForwardsToTargetInRegistry() throws Exception {
    RudpSessionManager forwarder =
        new RudpSessionManager(local, registry, metrics, (pub, payload) -> {});
  ...
  void relayResponseRejectedWithoutSession() {
    ...
    RudpSessionManager mgr =
        new RudpSessionManager(local, registry, metrics, (pub, payload) -> delivered.add(payload));
  ...
  void helloAckSetsInitiatorRemotePub() throws Exception {
    List<byte[]> delivered = new CopyOnWriteArrayList<>();
    RudpSessionManager mgr =
        new RudpSessionManager(local, registry, metrics, (pub, p) -> delivered.add(p));
    ...
    mgr.onPacket(
        new RudpPacketEnvelope(
            ...
            peer,
            new InetSocketAddress("127.0.0.1", 62100)));
  ...
  void helloRemainsPendingUntilValidSignedAck() throws Exception {
    ...
    mgr.connect(peer);
    assertEquals(1, mgr.pendingCountForTest(peer), "HELLO must be retransmittable");
    ...
    mgr.onPacket(
        new RudpPacketEnvelope(
            new RudpPacket(RudpPacket.Type.HELLO_ACK, wireCid, 0, 0, ackPayload),
            peer,
            new InetSocketAddress("127.0.0.1", 62090)));

    assertEquals(0, mgr.pendingCountForTest(peer), "signed HELLO_ACK must stop retransmits");
    assertTrue(mgr.hasRemotePubForTest(peer));
    mgr.shutdown();
  }

  @Test
  void authenticatedDataMigratesMobileNatEndpoint() throws Exception {
    RudpSessionManager mgr = new RudpSessionManager(local, registry, metrics, (pub, payload) -> {});
    long wireCid = 884422L;
    InetSocketAddress oldEndpoint = new InetSocketAddress("127.0.0.1", 62091);
    InetSocketAddress newEndpoint = new InetSocketAddress("127.0.0.1", 62092);
    byte[] helloPayload = RudpAuth.createHelloPayload(remote, wireCid);

    mgr.onPacket(
        new RudpPacketEnvelope(
            new RudpPacket(RudpPacket.Type.HELLO, wireCid, 0, 0, helloPayload),
            oldEndpoint,
            new InetSocketAddress("127.0.0.1", 62090)));
    assertEquals(oldEndpoint, mgr.remoteAddressForTest(wireCid));

    mgr.onPacket(
        new RudpPacketEnvelope(
            new RudpPacket(RudpPacket.Type.DATA, wireCid, 1, 0, new byte[] {1}),
            newEndpoint,
            new InetSocketAddress("127.0.0.1", 62090)));
  ...
  void pendingPacketsPurgedAfterRetransmitTimeout() throws Exception {
    ...
    mgr.onPacket(
        new RudpPacketEnvelope(
            new RudpPacket(RudpPacket.Type.HELLO, cid, 0, 0, helloPayload),
            sender,
            new InetSocketAddress("127.0.0.1", 62091)));
    assertEquals(1, mgr.sessionCount());
    ...
    assertTrue(
        packetsOutAfter - packetsOutBefore <= 1,
        "retransmissions should stop after timeout; delta=" + (packetsOutAfter - packetsOutBefore));
    mgr.shutdown();
  }
  ...
  void holePunchRejectedFromUnauthenticatedSender() throws Exception {
    ...
    assertEquals(
        0, mgr.sessionCount(), "HOLE_PUNCH from unauthenticated sender should be rejected");
    mgr.shutdown();
  }
