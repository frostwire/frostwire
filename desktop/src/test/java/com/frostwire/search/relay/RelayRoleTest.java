...
class RelayRoleTest {
  ...
  void constructorRejectsNullArgs() {
    assertThrows(IllegalArgumentException.class, () -> new RelayRole(null, directory));
    assertThrows(IllegalArgumentException.class, () -> new RelayRole(service, null));
  }
  ...
  void forwardSkipsPeersInPath() throws Exception {
    ...
    assertTrue(
        Arrays.equals(result.get(0).peerPub(), peerB.pub),
  ...
  private static byte[] fakePub(int i) {
  ...
  private static RemoteSearchRequest signRequest(KeyPairBag requester, String keywords, int limit)
      throws Exception {
