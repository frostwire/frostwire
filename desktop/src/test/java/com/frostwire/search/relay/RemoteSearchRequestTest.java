...
class RemoteSearchRequestTest {
    ...
    void builderAcceptsMinimalValid() {
        byte[] nonce = new byte[32];
        RemoteSearchRequest r = RemoteSearchRequest.builder()
                .nonce(nonce)
                .requesterPub(pubRaw)
                .signature(new byte[64])
                .build();
    ...
    void canonicalBytesAreDeterministic() {
        ...
        long ts = 1700000000L;
        RemoteSearchRequest a = RemoteSearchRequest.builder()
                .nonce(nonce)
                .requesterPub(pubRaw)
                .keywords("ubuntu")
                .limit(10)
                ...
                .signature(new byte[64])
                .build();
        RemoteSearchRequest b = RemoteSearchRequest.builder()
                .nonce(nonce)
                .requesterPub(pubRaw)
                .keywords("ubuntu")
                .limit(10)
    ...
    void isLoopDetectsPathMembership() {
        byte[] nonce = new byte[32];
        byte[] hop1 = new byte[32]; hop1[31] = 0x01;
        byte[] hop2 = new byte[32]; hop2[31] = 0x02;
        byte[] hop3 = new byte[32]; hop3[31] = 0x03;
        RemoteSearchRequest r = RemoteSearchRequest.builder()
                .nonce(nonce)
                .requesterPub(pubRaw)
    ...
    void roundTripThroughBencodeableMap() {
        ...
        assertEquals(Hex.encode(pubRaw),
