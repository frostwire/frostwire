...
public final class IceBridgeUrlHandler {
  ...
  public static synchronized void register() {
    ...
    if (OSUtils.isMacOSX()) {
      LOG.info(
          "icebridge:// OS registration on macOS requires a plist entry or native code; "
              + "add CFBundleURLTypes with CFBundleURLSchemes=icebridge to the app bundle Info.plist.");
    } else if (OSUtils.isWindows()) {
      LOG.info(
          "icebridge:// OS registration on Windows requires HKEY_CLASSES_ROOT\\icebridge "
              + "registry entry pointing to the FrostWire executable; not performed automatically.");
    } else {
      LOG.info(
          "icebridge:// OS registration on this platform requires manual desktop entry configuration.");
    }
    try {
  ...
  private static void handleBrowse(byte[] peerPub) {
    // Single browse UI for both entry points (URL handler, search-result action):
    // mesh-first with a DHT fallback.
    com.limegroup.gnutella.gui.icebridge.PeerCatalogWindow.showForPeer(peerPub);
  }
  ...
  private static void handleSearch(byte[] peerPub, String pubHex, String encodedKeywords) {
    ...
    if (distributed == null || !distributed.isEnabled() || !distributed.isReady()) {
      showError(
          I18n.tr("Distributed search engine is not ready. Please wait for IceBridge to start."));
      return;
    }
    ...
    JDialog loading =
        showLoading(I18n.tr("Searching peer {0}...", pubHex.substring(0, 12) + "..."));
    final byte[] pubFinal = peerPub;
    final String kwFinal = keywords.trim();
    SwingWorker<List<RemoteSearchResponse.Row>, Void> worker =
        new SwingWorker<>() {
          @Override
                  ...
                  (sourcePub, responsePayload, receivedMs) -> {
                    try {
                      ...
                      RemoteSearchResponse response =
                          SearchPayloadCodec.decodeResponse(responsePayload);
                      if (response == null) {
                        return;
                      }
          ...
          protected void done() {
            ...
            try {
              ...
              if (rows == null) {
                showError(
                    I18n.tr(
                        "Failed to send search request to peer {0}. The peer may be offline.",
                        pubHex.substring(0, 12) + "..."));
                return;
              }
              if (rows.isEmpty()) {
                JOptionPane.showMessageDialog(
                    ...
                    I18n.tr(
                        "No results from peer {0} for \"{1}\".",
                        pubHex.substring(0, 12) + "...", kwFinal),
              ...
              sb.append("<tr><td><b>")
                  ...
                  .append(I18n.tr("Size"))
                  .append("</b></td><td><b>")
                  .append(I18n.tr("Files"))
                  .append("</b></td></tr>");
              for (RemoteSearchResponse.Row row : rows) {
                sb.append("<tr><td>")
                    .append(escapeHtml(row.name))
                    .append("</td><td>")
                    .append(humanSize(row.sizeBytes))
                    .append("</td><td>")
                    .append(row.fileCount)
                    .append("</td></tr>");
              }
            ...
            try {
              ...
              if (directory != null) {
                ...
                if (info.isPresent()) {
                  ...
                  sb.append("<html>");
                  sb.append("<b>")
                      .append(I18n.tr("Public Key"))
                      .append(":</b> ")
                      .append(pubHex)
                      .append("<br>");
                  sb.append("<b>")
                      .append(I18n.tr("Hostname"))
                      .append(":</b> ")
                      .append(escapeHtml(p.hostname()))
                      .append("<br>");
                  sb.append("<b>")
                      .append(I18n.tr("Port"))
                      .append(":</b> ")
                      ...
                      .append("<br>");
                  sb.append("<b>")
                      .append(I18n.tr("Verified"))
                      .append(":</b> ")
                      .append(p.isVerified() ? I18n.tr("Yes") : I18n.tr("No"))
                      .append("<br>");
                  sb.append("<b>")
                      .append(I18n.tr("Endorsers"))
                      .append(":</b> ")
                      .append(p.endorserCount())
                      .append("<br>");
                  sb.append("<b>")
                      .append(I18n.tr("Spam"))
                      .append(":</b> ")
                      .append(p.isSpam() ? I18n.tr("Yes") : I18n.tr("No"))
                      .append("<br>");
                  sb.append("<b>")
                      .append(I18n.tr("Last Updated"))
                      .append(":</b> ")
                      .append(new java.util.Date(p.lastUpdatedMs()))
                      .append("<br>");
                  sb.append("<b>")
                      .append(I18n.tr("Trust Score"))
                      .append(":</b> ")
                      .append(String.format("%.2f", directory.trustScore(peerPub)));
                  sb.append("</html>");
                  return sb.toString();
                }
              }
              ...
              sb.append("<b>")
                  ...
                  .append("<br>");
              sb.append("<b>")
                  .append(I18n.tr("Node ID"))
                  .append(":</b> ")
                  .append(Hex.encode(record.nodeId()))
                  .append("<br>");
              sb.append("<b>")
                  .append(I18n.tr("uTP Port"))
                  .append(":</b> ")
                  .append(record.utpPort())
                  .append("<br>");
              sb.append("<b>")
                  .append(I18n.tr("First Seen"))
                  .append(":</b> ")
                  .append(new java.util.Date(record.firstSeen() * 1000L))
                  .append("<br>");
              sb.append("<b>")
                  .append(I18n.tr("Last Seen"))
                  .append(":</b> ")
                  .append(new java.util.Date(record.lastSeen() * 1000L))
                  .append("<br>");
              sb.append("<b>")
                  .append(I18n.tr("Signature Valid"))
                  .append(":</b> ")
                  .append(record.verifySignature() ? I18n.tr("Yes") : I18n.tr("No"));
              sb.append("<br><i>")
                  .append(I18n.tr("(Fetched from DHT; peer not in local directory)"))
                  .append("</i>");
              sb.append("</html>");
              return sb.toString();
            } catch (Throwable t) {
            ...
            try {
              ...
              if (info == null) {
                JOptionPane.showMessageDialog(
                    GUIMediator.getAppFrame(),
                    I18n.tr(
                        "Peer {0} is not known and no identity record was found in the DHT.",
                        pubHex.substring(0, 12) + "..."),
  ...
  private static RemoteSearchRequest buildSignedRequest(String keywords, IdentityKeys identity)
      throws Exception {
  ...
  private static final class ClipboardMonitor extends WindowAdapter {
    ...
    public void windowActivated(WindowEvent e) {
      try {
        ...
        if (content == null
            || !content.isDataFlavorSupported(java.awt.datatransfer.DataFlavor.stringFlavor)) {
          return;
        }
        String text =
            (String) content.getTransferData(java.awt.datatransfer.DataFlavor.stringFlavor);
        if (text == null) {
          return;
        }
        text = text.trim();
