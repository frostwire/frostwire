...
import com.limegroup.gnutella.gui.icebridge.IceBridgeConsoleWindow;
...
final class ToolsMenu extends AbstractMenu {
  ...
  /**
   * Creates a new `ToolsMenu`, using the `key` argument for setting the locale-specific title and
   * accessibility text.
   */
  ToolsMenu() {
    super(I18n.tr("&Tools"));
    ...
    JRadioButtonMenuItem darkItem =
        new JRadioButtonMenuItem(
            new AbstractAction(I18n.tr("&Dark Flat (beta)")) {
              @Override
              public void actionPerformed(ActionEvent e) {
  ...
  private static class IceBridgeConsoleAction extends AbstractAction {
    IceBridgeConsoleAction() {
      super(I18n.tr("IceBridge &Console"));
      putValue(LONG_DESCRIPTION, I18n.tr("Show live IceBridge and distributed search events"));
    }

    @Override
    public void actionPerformed(ActionEvent e) {
      IceBridgeConsoleWindow.showConsole();
    }
  }
  ...
  private static class UpdateAction extends AbstractAction {
    ...
    void refresh() {
      ...
      if (UpdateMediator.instance().isUpdated()) {
        ...
        text =
            I18n.tr("Downloading update...")
                + "("
                + UpdateMediator.instance().getUpdateDownloadProgress()
                + "%)";
        enabled = false;
