...
/**
 * The component for the space at the bottom of the main application window, including the connected
 * status and the media player.
 */
public final class StatusLine implements VPNStatusRefresher.VPNStatusListener {
  ...
  /**
   * The left most panel containing the connection quality. The switcher changes the actual
   * ImageIcons on this panel.
   */
  ...
  private final MouseAdapter STATUS_BAR_LISTENER =
      new MouseAdapter() {
        ...
        void processMouseEvent(MouseEvent me) {
          ...
          if (me.isPopupTrigger()) {
            ...
            if (clickedComponent == bandwidthUsageUp || clickedComponent == bandwidthUsageDown) {
              final GUIMediator.Tabs transfersTab =
                  GUIMediator.Tabs.TRANSFERS.isEnabled()
                      ? GUIMediator.Tabs.TRANSFERS
                      : GUIMediator.Tabs.SEARCH_TRANSFERS;
  ...
  /**
   * Variables for the center portion of the status bar, which can display the StatusComponent
   * (progress bar during program load), the UpdatePanel (notification that a new version of
   * FrostWire is available), and the StatusLinkHandler (ads for going PRO).
   */
    ...
    SwingUtilities.invokeLater(
        () -> {
          ...
          GUIMediator.setSplashScreenString(
              I18n.tr("Creating donation buttons so you can give us a hand..."));
          ...
          vpnStatusRefresher.addRefreshListener(
              (vpnIsOn) -> {
          ...
          GUIMediator.setSplashScreenString(
              I18n.tr("Playing with pixels for the Firewall indicator..."));
          createFirewallLabel();
  ...
  private void createTwitterButton() {
    ...
    initSocialButton(
        twitterButton, I18n.tr("Follow us @frostwire"), GUIConstants.TWITTER_FROSTWIRE_URL);
  }
  ...
  private void createInstagramButton() {
    ...
    initSocialButton(
        instagramButton,
        I18n.tr("Follow FrostWire on Instagram"),
        GUIConstants.INSTAGRAM_FROSTWIRE_URL);
  ...
  private void createFacebookButton() {
    ...
    initSocialButton(
        facebookButton,
        I18n.tr(
            "Like FrostWire on Facebook and stay in touch with the community. Get Help and Help Others."),
        GUIConstants.FACEBOOK_FROSTWIRE_URL);
  }
  ...
  private void createDiscordButton() {
    ...
    initSocialButton(
        discordButton,
        I18n.tr("Join the FrostWire community on Discord"),
        GUIConstants.FROSTWIRE_CHAT_URL);
  }
  ...
  private void createSeedingStatusLabel() {
    seedingStatusButton =
        new IconButton("", "SEEDING", true) {
          @Override
          public String getToolTipText() {
            boolean seedingStatus = SharingSettings.SEED_FINISHED_TORRENTS.getValue();
            return "<html>"
                + (seedingStatus
                    ? I18n.tr("<b>Seeding</b><p>completed torrent downloads.</p>")
                    : I18n.tr(
                            "<b>Not Seeding</b><p>File chunks might be shared only during a torrent download.</p>")
                        + "</html>");
          }
        };
    ...
    seedingStatusButton.addActionListener(
        e ->
            GUIMediator.instance()
                .setOptionsVisible(true, OptionsConstructor.BITTORRENT_BASIC_KEY));
    ToolTipManager.sharedInstance().registerComponent(seedingStatusButton);
  }

  /**
   * Redraws the status bar based on changes to StatusBarSettings, and makes sure it has room to add
   * an indicator before adding it.
   */
  public void refresh() {
    ...
    if (getComponent() != null) {
      ...
      if (remainingWidth <= 0) remainingWidth = ApplicationSettings.APP_WIDTH.getValue();
      ...
      if (StatusBarSettings.VPN_DROP_PROTECTION_DISPLAY_ENABLED.getValue()
          && vpnDropProtectionCheckbox != null) {
        remainingWidth =
            addStatusIndicator(vpnDropProtectionCheckbox, sepWidth, remainingWidth, gbc);
      }
      ...
      indicatorWidth =
          GUIConstants.SEPARATOR
              + GUIConstants.SEPARATOR / 2
              + sepWidth
              + Math.max(
                  (int) bandwidthUsageDown.getMinimumSize().getWidth(),
                  bandwidthUsageDown.getWidth())
              + Math.max(
                  (int) bandwidthUsageUp.getMinimumSize().getWidth(), bandwidthUsageUp.getWidth());
      if (StatusBarSettings.BANDWIDTH_DISPLAY_ENABLED.getValue()
          && remainingWidth > indicatorWidth) {
      ...
      // donation buttons
      if (donationButtons != null
          && StatusBarSettings.DONATION_BUTTONS_DISPLAY_ENABLED.getValue()) {
        BAR.add(Box.createHorizontalStrut(GUIConstants.SEPARATOR / 2), gbc);
  ...
  private int addStatusIndicator(
      JComponent component, int sepWidth, int remainingWidth, GridBagConstraints gbc) {
    int indicatorWidth;
    indicatorWidth =
        GUIConstants.SEPARATOR
            + Math.max((int) component.getMinimumSize().getWidth(), component.getWidth())
            + sepWidth;
  ...
  /** _lwsStatus = new JLabel(); Sets up the 'Bandwidth Usage' label. */
  private void createBandwidthLabel() {
    // Load icons (this was deferred to avoid EDT blocking with file I/O)
    ImageIcon downloadIcon =
        (ImageIcon)
            IconRepainter.brightenIfDarkTheme(GUIMediator.getThemeImage("downloading_small"));
    ImageIcon uploadIcon =
        (ImageIcon) IconRepainter.brightenIfDarkTheme(GUIMediator.getThemeImage("uploading_small"));
  ...
  private void createCenterPanel() {
    ...
    STATUS_COMPONENT.setText(I18n.tr("Preparing FrostWire..."));
  ...
  /**
   * Updates the center panel if non-PRO. Periodically rotates between the update panel and the
   * status link handler.
   */
  ...
  private void updateSeedingStatus() {
    ...
    if (seedingStatus != previousSeedingStatus) {
      ...
      ImageIcon newIcon =
          seedingStatus
              ? GUIMediator.getThemeImage("seeding_small")
              : GUIMediator.getThemeImage("not_seeding_small");
  ...
  private void updateBandwidth() {
    try {
      ...
      String sDown =
          GUIUtils.rate2speed(
              GUIMediator.instance().getBTDownloadMediator().getDownloadsBandwidth());
      String sUp =
          GUIUtils.rate2speed(GUIMediator.instance().getBTDownloadMediator().getUploadsBandwidth());
   ...
   *
   * <p>The loading label is removed and the update notification component is added. If necessary,
   * the center panel will rotate back and forth between displaying the update notification and
   * displaying the StatusLinkHandler.
   */
  void loadFinished() {
    updateCenterPanel();
    if (centerPanel != null) {
  ...
  private void updateTheme() {
    try {
      _connectionQualityMeterIcons[StatusLine.STATUS_DISCONNECTED] =
          GUIMediator.getThemeImage("connect_small_0");
      _connectionQualityMeterIcons[StatusLine.STATUS_TURBOCHARGED] =
          GUIMediator.getThemeImage("connect_small_6");
   ...
   *
   * @return the `JComponent` instance that contains all of the panels for the status line
   */
