 ...
 *
 * <p>The panes that show up when a leaf in the tree is selected are created lazily in {@link
 * OptionsPaneFactory}.
 *
 * <p>If you want to add a new {@link OptionsPane}, add a call to {@link
 * OptionsConstructor#addOption} in the constructor here and add the construction of the pane to
...
public final class OptionsConstructor {
  ...
  /** Handle to the top-level `JDialog</tt window that contains all of the other GUI components. */
  private final JDialog DIALOG;

  /** Stored for convenience to allow using this in helper methods during construction. */
  ...
  /**
   * The constructor create all the options windows and their components.
   *
   * @param treeManager the `OptionsTreeManager` instance to use for constructing the main panels
   *     and adding elements
   * @param paneManager the `OptionsPaneManager` instance to use for constructing the main panels
   *     and adding elements
   */
  ...
  private OptionsTreeNode initializePanels() {
    ...
    addOption(
        ...
        TorrentFilesFolderPaneItem.class,
    ...
    // library
    // REMOVE:WI-FI
    // addOption(OptionsMediator.ROOT_NODE_KEY, LIBRARY_KEY, I18n.tr("Library"),
    // LibraryFoldersPaneItem.class, WiFiSharingPaneItem.class, LibraryInternetRadioPaneItem.class);
    addOption(
    ...
    OptionsTreeNode node =
        keysToNodes.get(ApplicationSettings.OPTIONS_LAST_SELECTED_KEY.getValue());
    if (node == null) {
   ...
   * corresponding `OptionsPane`. This method allows for multiple tiers of parent nodes, not only
   * top-level parents.
   *
   * @param parentKey the key of the parent node to add this parent node to
   * @param childKey the key of the new parent node that is a child of the `parentKey` argument
   */
  ...
  /**
   * Adds the specified key and `OptionsPane` to current set of options. This adds this
   * `OptionsPane` to the set of `OptionsPane`s the user can select.
   *
   * @param parentKey the key of the parent node to add the new node to
   */
  @SafeVarargs
  private OptionsTreeNode addOption(
      final String parentKey,
      final String childKey,
      final String label,
      Class<? extends AbstractPaneItem>... clazzes) {
    OptionsTreeNode node =
        TREE_MANAGER.addNode(parentKey, childKey, label, label + " " + extractLabels(clazzes));
   ...
   *
   * @param visible `boolean` value specifying whether the options window should be made visible or
   *     not visible
  ...
  void setOptionsVisible(boolean visible, final String key) {
    if (!visible) {
      ...
      DIALOG.pack();
      DIALOG.setSize(
          UISettings.UI_OPTIONS_DIALOG_WIDTH.getValue(),
          UISettings.UI_OPTIONS_DIALOG_HEIGHT.getValue());
  ...
  /**
   * Returns the main `JDialog` instance for the options window, allowing other components to
  ...
  /**
   * Inner class that computes meaningful default dialog sizes for the options dialog for different
   * font size increments.
   *
   * <p>It also updates the width and height setting if the user changes the dialog size manually.
   */
