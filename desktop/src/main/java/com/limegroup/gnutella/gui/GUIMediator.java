...
/**
 * This class acts as a central point of access for all gui components, a sort of "hub" for the
 * frontend. This should be the only common class that all frontend components have access to,
 * reducing the overall dependencies and therefore increasing the modularity of the code.
 *
 * <p>Any functions or services that should be accessible to multiple classes should be added to
 * this class. These currently include such functions as easily displaying standardly-formatted
 * messages to the user, obtaining locale-specific strings, and obtaining image resources, among
 * others.
 *
 * <p>All of the methods in this class should be called from the event- dispatch (Swing) thread.
 */
public final class GUIMediator {
  private static final ThreadPool pool =
      new ThreadPool("GUIMediator-background-pool", 1, 1, 60, new LinkedBlockingQueue<>(), true);

  /** Message key for the disconnected message */
  private static final String DISCONNECTED_MESSAGE =
      I18n.tr(
          "Your machine does not appear to have an active Internet connection or a firewall is blocking FrostWire from accessing the internet. FrostWire will automatically keep trying to connect you to the network unless you select \"Disconnect\" from the File menu.");

  /** `List` of `RefreshListener` classes to notify of UI refresh events. */
  ...
  /**
   * Flag for whether or not a message has been displayed to the user -- useful in deciding whether
   * or not to display other dialogues.
   */
  ...
  /**
   * Handle to the `OptionsMediator` class that is responsible for displaying customizable options
   * to the user.
   */
  ...
  /** Flag for whether the app has ever been made visible during this session. */
  ...
  /**
   * Constant handle to the `MainFrame` instance that handles constructing all of the primary gui
   * components.
   */
  private final MainFrame MAIN_FRAME;

  /**
   * Constant handle to the `DownloadMediator` class that is responsible for displaying active
   * downloads to the user.
   */
  private BTDownloadMediator BT_DOWNLOAD_MEDIATOR;

  /**
   * Constant handle to the `DownloadView` class that is responsible for displaying the status of
   * the network and connectivity to the user.
   */
  ...
  /** Private constructor to ensure that this class cannot be constructed from another class. */
  private GUIMediator() {
  ...
  public static synchronized GUIMediator instance() {
    if (_instance == null) _instance = new GUIMediator();
    return _instance;
  }
   ...
   *
   * @return `true` if the `JFrame` is visible, `false` otherwise
   */
   ...
   *
   * @param visible specifies whether or not the application should be made visible or not
   */
  static void setAppVisible(final boolean visible) {
    safeInvokeLater(
        () -> {
          try {
            if (visible) getAppFrame().toFront();
            getAppFrame().setVisible(visible);
          } catch (NullPointerException npe) {
            System.out.println("GUIMediator - NULL POINTER EXCEPTION HAPPENED");
            if (visible) FatalBugManager.handleFatalBug(npe);
            else ErrorService.error(npe);
          } catch (Throwable t) {
            if (visible) FatalBugManager.handleFatalBug(t);
  ...
  /**
   * Displays various dialog boxes that should only be shown the first time the application is made
   * visible.
   */
  ...
  /**
   * Closes any dialogues that are displayed at startup and sets the flag to indicate that we've
   * displayed a message.
   */
  static void closeStartupDialogs() {
    if (SplashWindow.instance().isShowing()) SplashWindow.instance().toBack();
    if (TipOfTheDayMediator.isConstructed()) TipOfTheDayMediator.instance().hide();
  }

  /**
   * Returns a `Dimension` instance containing the dimensions of the wrapped JFrame.
   *
   * @return a `Dimension` instance containing the width and height of the wrapped JFrame
   */
  ...
  /**
   * Returns a `Point` instance containing the x, y position of the wrapped `JFrame` on the screen.
   *
   * @return a `Point` instance containing the x, y position of the wrapped JFrame
   */
   ...
   *
   * @return `true` if the options window is visible, `false` otherwise
   */
  ...
  public void setOptionsVisible(boolean visible) {
    if (OPTIONS_MEDIATOR == null) return;
   ...
   *
   * @return the options window main `JComponent`, or `null` if the options window has not yet been
   *     constructed (the window is guaranteed to be constructed if it is visible)
   */
  static Component getMainOptionsComponent() {
    if (OPTIONS_MEDIATOR == null) return null;
  ...
  public static ShellAssociationManager getAssociationManager() {
    if (ASSOCIATION_MANAGER == null) {
      ASSOCIATION_MANAGER =
          new ShellAssociationManager(FrostAssociations.getSupportedAssociations());
    }
  ...
  static void startupHidden() {
    ...
    if (OSUtils.isMacOSX()) GUIMediator.hideView();
  }
  ...
  /**
   * Handles a 'reopen' event appropriately. Used primarily for allowing LimeWire to be made visible
  ...
  static void handleReopen() {
    ...
    // is not yet finished.
    if (_allowVisible) {
      if (!_visibleOnce) restoreView(); // First make sure it's not minimized
  ...
  /**
   * Hides the GUI by either sending it to the System Tray or minimizing the window. Minimize
   * behavior occurs on platforms which do not support the System Tray.
   *
  ...
  public static void restoreView() {
    ...
    if (OSUtils.supportsTray() && ResourceManager.instance().isTrayIconAvailable()) {
      ...
      if (ApplicationSettings.DISPLAY_TRAY_ICON.getValue()) GUIMediator.showTrayIcon();
      else GUIMediator.hideTrayIcon();
      if (doHack) restoreView();
    }
    getAppFrame().setState(Frame.NORMAL);
  }

  /**
   * Determines the appropriate shutdown behavior based on user settings. This implementation
   * decides between exiting the application immediately, or exiting after all file transfers in
  ...
  /**
   * Restarts the application by spawning a new JVM process with the same classpath, main class, and
   * JVM arguments, then shuts down the current instance.
   */
  ...
  /**
   * Shows a dialog informing the user that a restart is required and offers a "Restart Now" button.
   */
  ...
  /**
   * Shows the user notification area. The user notification icon and tooltip created by the
   * NotifyUser object are not modified.
   */
  ...
  /**
   * Sets the window height, width and location properties to remember the next time the program is
   * started.
   */
  static void applyWindowSettings() {
    ApplicationSettings.RUN_ONCE.setValue(true);
    if (GUIMediator.isAppVisible()) {
      if ((GUIMediator.getAppFrame().getExtendedState() & Frame.MAXIMIZED_BOTH)
   ...
   *
   * @param name the name of the icon to return without path information, as in "plug"
   ...
   *
   * <p>The `messageKey` parameter must be the key for a locale- specific message `String` and not a
   * hard-coded value.
   *
   * @param message the locale-specific message to display
   * @return an integer indicating a yes or a no response from the user
   */
  public static DialogOption showYesNoMessage(
   ...
   */
  public static DialogOption showYesNoMessage(
      final String message, final IntSetting defaultValue, final DialogOption defaultOption) {
    return MessageService.instance().showYesNoMessage(message, defaultValue, defaultOption);
  }

  @SuppressWarnings("unused")
  public static DialogOption showYesNoTitledMessage(
      final String message, final String title, final DialogOption defaultOption) {
    return MessageService.instance().showYesNoMessage(message, title, defaultOption);
  }

  /**
   * Displays a locale-specific message to the user in the form of a yes/no/{other} question.
   *
   * <p>The `messageKey` parameter must be the key for a locale- specific message `String` and not a
   * hard-coded value.
   *
   ...
   * @return an integer indicating a yes or a no response from the user
   */
  public static DialogOption showYesNoOtherMessage(
  ...
  /**
   * Acts as a proxy for the `MessageService` class. Displays a locale-specific message to the user
   * in the form of a yes or no or cancel question.
   *
   * <p>The `messageKey` parameter must be the key for a locale- specific message `String` and not a
   * hard-coded value.
   *
   ...
   */
  @SuppressWarnings("unused")
  public static DialogOption showYesNoCancelMessage(
   ...
   *
   * @param message the locale-specific message to display
   * @param ignore the BooleanSetting that stores/retrieves whether or not to display this message.
   */
  ...
  /**
   * Acts as a proxy for the `MessageService` class. Displays a locale-specific disposable message
   * to the user.
   *
   * <p>The `messageKey` parameter must be the key for a locale- specific message `String` and not a
   * hard-coded value.
   ...
   */
  private static void showDisposableMessage(
      @SuppressWarnings("SameParameterValue") final String messageKey,
      final String message,
      @SuppressWarnings("SameParameterValue") final Switch ignore,
      @SuppressWarnings("SameParameterValue") int msgType) {
    MessageService.instance().showDisposableMessage(messageKey, message, ignore, msgType);
  }

  /**
   * Acts as a proxy for the `MessageService` class. Hides a locale-specific disposable message.
   *
   * <p>The `messageKey` parameter must be the key for a locale- specific message `String` and not a
   * hard-coded value.
   *
   * @param messageKey the key for the locale-specific message to display
   */
  private static void hideDisposableMessage(
      @SuppressWarnings("SameParameterValue") final String messageKey) {
   ...
   * @param ignore the BooleanSetting for that stores/retrieves whether or not to display this
   *     message.
   */
  ...
  /**
   * Acts as a proxy for the `MessageService` class. Displays a locale-specific warning message to
   * the user.
   *
   * <p>The `messageKey` parameter must be the key for a locale- specific message `String` and not a
   * hard-coded value.
  ...
  /**
   * Acts as a proxy for the Launcher class so that other classes only need to know about this
   * mediator class.
   *
    ...
    try {
      ...
      if (OSUtils.isLinux()) {
        GUIMediator.showError(
            I18n.tr(
                "Cannot open URLs. Try installing GVFS and restarting FrostWire to solve the issue."));
      } else {
      ...
      GUIMediator.showError(
          I18n.tr(
              "FrostWire could not locate your web browser to display the following web page: {0}.",
              url));
      return -1;
    }
  }
   ...
   *
   * @param file a `File` instance denoting the abstract pathname of the file to launch
   */
   ...
   * mediator class.
   *
   * <p>Opens `file` in a platform specific file manager.
   *
   * @param file a `File` instance denoting the abstract pathname of the file to launch
   */
  ...
  /**
   * Modifies the text displayed to the user in the splash screen to provide application loading
   * information.
   *
   * @param text the text to display
   */
  public static void setSplashScreenString(String text) {
    if (!_allowVisible) SplashWindow.instance().setStatusText(text);
    else if (isConstructed()) instance().getStatusLine().setStatusText(text);
  }

  /**
   * Returns the point for the placing the specified component on the center of the screen.
   *
   * @param comp the `Component` to use for getting the relative center point
   * @return the `Point` for centering the specified `Component` on the screen
   */
  ...
  /**
   * Adds the specified `RefreshListener` instance to the list of listeners to be notified when a UI
  ...
  public static void addRefreshListener(RefreshListener listener) {
    if (!REFRESH_LIST.contains(listener)) REFRESH_LIST.add(listener);
  }
  ...
  /**
   * Returns the `MainFrame` instance. `MainFrame` maintains handles to all of the major gui
   * classes.
   *
  ...
  /**
   * Sets the visibility state of the options window, and sets the selection to a option pane
  ...
  /**
   * Sets the tab pane to display the given tab. Tip: Try to leave this method call last on your
   * actions.
   *
  ...
  public void setWindow(GUIMediator.Tabs tabEnum) {
    ...
    // If we've never selected a directory holder in the library, then we have it select "Default
    // Save Folder"
    if (LibrarySettings.LAST_SELECTED_LIBRARY_DIRECTORY_HOLDER_OFFSET.getValue() == -1) {
      ...
      LibraryMediator.instance()
          .getLibraryExplorer()
          .selectDirectoryHolderAt(
              LibrarySettings.LAST_SELECTED_LIBRARY_DIRECTORY_HOLDER_OFFSET.getValue());
    }
  }

  /**
   * If the window to be shown is the Library tab, we automatically select "Finished Downloads" so
   * the users have a clue of what they can do with the Library, and so that they see their finished
   * downloads in case they came here the first time to see what they downloaded.
   */
  ...
  /**
   * Notification that loading is finished. Updates the status line and bumps the AWT thread
   * priority.
   */
  void loadFinished() {
    SwingUtilities.invokeLater(
   ...
   *
   * @param cursor the cursor that should be shown on the frame and all its child components that
   *     don't have their own cursor set
   */
  ...
  public void openSoundcloudTrackUrl(
      String trackUrl, SoundcloudSearchResult sr, boolean fromPastedUrl) {
    showTransfers(TransfersTab.FilterMode.ALL);
    getBTDownloadMediator()
        .downloadSoundcloudFromTrackUrlOrSearchResult(trackUrl, sr, fromPastedUrl);
  }
  ...
  public void openHttp(
      ...
      final long fileSize,
      final boolean extractAudioAndDeleteOriginal) {
    openHttp(httpUrl, title, saveFileAs, fileSize, extractAudioAndDeleteOriginal, null);
  }
    ...
    Tabs(String nameWithAmpersand) {
      navAction =
          new NavigationAction(
              nameWithAmpersand,
              I18n.tr("Display the {0} Screen", GUIUtils.stripAmpersand(nameWithAmpersand)));
    }
