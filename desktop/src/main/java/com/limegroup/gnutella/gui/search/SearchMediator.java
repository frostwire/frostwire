...
/**
 * This class acts as a mediator between the various search components -- the hub that all traffic
 * passes through. This allows the decoupling of the various search packages and simplifies the
 * responsibilities of the underlying classes.
 */
...
public final class SearchMediator {
  ...
  private SearchMediator() {
    ...
    SwingUtilities.invokeLater(
        () -> {
          ...
          getSearchResultDisplayer()
              .setSearchListener(
                  e -> {
                    SearchResultMediator resultPanel =
                        getSearchResultDisplayer().getSelectedResultPanel();
                    if (resultPanel != null) {
  ...
  public void updateSearchPanelTitle(final long token, final String title) {
    GUIMediator.safeInvokeLater(
        () -> {
          SearchResultMediator resultsPanel =
              getSearchResultDisplayer().getResultPanelForGUID(token);
          if (resultsPanel == null) {
            LOG.info(
                "updateSearchPanelTitle: could not find SearchResultMediator for token "
                    + token
                    + ", closed prematurely perhaps");
            return;
          }
  ...
  private static boolean validate(SearchInformation info) {
    switch (validateInfo(info)) {
      ...
      case QUERY_TOO_SHORT:
        GUIMediator.showMessage(
            I18n.tr(
                "Your search must be at least three characters to avoid congesting the network."));
        return false;
      case QUERY_TOO_LONG:
        GUIMediator.showMessage(
            I18n.tr("Your search is too long. Please make your search smaller and try again."));
        return false;
      default:
        return true;
    }
  ...
  /** Validates the a search info and returns {@link #QUERY_VALID} if it is valid. */
  ...
  private static List<UISearchResult> convertResults(
      List<? extends SearchResult> results, SearchEngine engine, String query) {
    ...
    for (SearchResult sr : results) {
      ...
      if (sr instanceof SoundcloudSearchResult) {
        ...
        ui =
            new InternetArchiveUISearchResult(
                (InternetArchiveCrawledSearchResult) sr, engine, query);
        ...
        if (tsr.getDownloadUrl() == null) {
          ...
          String extension = FilenameUtils.getExtension(tsr.getFilename());
          if (extension.equals("m4a")) {
            TellurideSearchResult fasterAudio =
                new TellurideSearchResult(
                    tsr.getId(),
                    "(Faster Download) " + tsr.getDisplayName() + " (.m4a)",
                    tsr.getFilename(),
                    tsr.getSource(),
                    tsr.getDetailsUrl(),
                    tsr.getDownloadUrl(),
  ...
  /**
   * Adds a single result tab for the specified GUID, type, standard query string, and XML query
   * string.
   */
  ...
  /** Downloads the selected files in the currently displayed `ResultPanel` if there is one. */
  ...
  private static void downloadAll(SearchResultDataLine[] lines) {
    ...
    if (lines.length == 1) {
      ...
      if (!sr.isPreliminary()
          && !(uiSearchResult instanceof TelluridePartialUISearchResult)
          && !(uiSearchResult instanceof TelluridePlaylistPartialUISearchResult)) {
  ...
  /**
   * called by ResultPanel when the views are changed. Used to set the tab to indicate the correct
   * number of TableLines in the current view.
   */
   ...
   *
   * @return the `ResultPanel` that matches the GUID, or null if none match.
   */
  ...
  /**
   * Returns the `JComponent` instance containing all of the search result UI components. Returns a
   * placeholder immediately to avoid blocking EDT. The real component is created asynchronously.
   *
   * @return the `JComponent` instance containing all of the search result UI components
   */
          ...
          () -> {
            synchronized (SearchMediator.class) {
              if (RESULT_DISPLAYER == null && RESULT_COMPONENT_PLACEHOLDER != null) {
                ...
                RESULT_COMPONENT_PLACEHOLDER.add(
                    RESULT_DISPLAYER.getComponent(), BorderLayout.CENTER);
   ...
   *
   * <p>Returns the GUID of the search if a search was initiated, otherwise returns null.
   */
  ...
  private void performSearch(final long token, String query) {
    ...
    if (query.startsWith("http") && !query.endsWith(".torrent")) {
      manager.perform(
          SearchEngine.getSearchEngineByID(SearchEngine.SearchEngineID.FROSTCLICK_ID)
              .getPerformer(token, query));
      boolean isPlaylist = isYouTubePlaylistUrl(query);
      manager.perform(
          SearchEngine.getSearchEngineByID(SearchEngine.SearchEngineID.TELLURIDE_ID)
              .getPerformer(token, query, isPlaylist));
      return;
    }
  ...
  private List<SearchResult> filter2(
      List<? extends SearchResult> results, List<String> searchTokens) {
    ...
    try {
      ...
      // possible NPE due to cancel search or some inner error in search results, ignore it and
      // cleanup list
      list.clear();
    }
    return list;
   ...
   *
   * <p>Strips control characters (except common whitespace), null bytes, and limits length to 1024
   * characters to prevent injection into URL templates, regex patterns, and JSON request bodies.
   */
  ...
  private void onResults(final long token, List<? extends SearchResult> results) {
    ...
    boolean isTellurideSearchResult =
        results.get(0).getSource().startsWith("Cloud:"); // will be stopped
    if (rp != null && (isTellurideSearchResult || !rp.isStopped())) {
      List<SearchResult> filtered;
      if (isYouTubePlaylistUrl(rp.getQuery())) {
        @SuppressWarnings("unchecked")
        List<SearchResult> r = (List<SearchResult>) results;
        filtered = r;
      } else {
        @SuppressWarnings("unchecked")
        List<SearchResult> f = filter((List<SearchResult>) results, rp.getSearchTokens());
        filtered = f;
      }
            ...
            () -> {
              try {
                ...
                if (firstMediaType != null && added > 0) {
                  LOG.info(
                      "Auto-selecting media type tab: "
                          + firstMediaType.getName()
                          + " for result count="
                          + added);
