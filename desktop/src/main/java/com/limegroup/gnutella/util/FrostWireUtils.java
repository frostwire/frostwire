...
public final class FrostWireUtils {
  ...
  private static final String FROSTWIRE_VERSION = "7.1.0";

  /** Build number for the current version, gets reset to 1 on every version bump */
  private static final int BUILD_NUMBER = 332;
  ...
  /** Returns the current version number of FrostWire as a string, e.g., "5.2.9". */
  ...
  /**
   * Returns the path of the FrostWire.jar executable. For a windows binary distribution this is the
   * same path as FrostWire.exe since those files live together.
   */
  public static String getFrostWireJarPath() {
    return new File(
            FrostWireUtils.class.getProtectionDomain().getCodeSource().getLocation().getPath())
        .getParent();
  }

  /** Returns the root folder from which all Saved/Shared/etc.. folders should be placed. */
  public static File getFrostWireRootFolder() {
    String root = null;
    if (OSUtils.isWindows()) {
      root = SystemUtils.getSpecialPath(SpecialLocations.DOCUMENTS);
    }
    if (root == null || "".equals(root)) root = CommonUtils.getUserHomeDir().getPath();
  ...
  public static File getUserVideoFolder() {
    return (OSUtils.isMacOSX())
        ? new File(CommonUtils.getUserHomeDir(), "Movies")
        : // Videos dir for macOS
  ...
  /**
   * Determines the path of the telluride launcher relative to FrostWire and returns the
   * corresponding File object. It should determine it regardless of running from source or from a
   * binary distribution.
   */
  public static File getTellurideLauncherFile() {
    boolean isRelease =
        !FrostWireUtils.getFrostWireJarPath()
            .contains("frostwire" + File.separatorChar + "desktop");
    if (isRelease) {
      if (OSUtils.isWindows()) {
        ...
        // System.out.println("FrostWireUtils.getTellurideLauncherFile(): java.home -> " +
        // javaHome);
        // FrostWireUtils.getTellurideLauncherFile(): java.home ->
        // /Path/To/FrostWire.app/Contents/PlugIns/jre/Contents/Home
        ...
        f = f.getParentFile(); // Contents
        return new File(
            f, "MacOS" + File.separator + "telluride_macos." + OSUtils.getMacOSArchitecture());
      ...
      String pathPrefix =
          getDevelopmentFrostWireDesktopFolderPath()
              + File.separatorChar
              + ".."
              + File.separatorChar
              + "telluride";
