...
public class SettingsSetTool implements MCPTool {
  ...
  public JsonObject inputSchema() {
    ...
    keyProp.addProperty(
        "description", "The setting key name (e.g. VPN_DROP_PROTECTION, SEED_FINISHED_TORRENTS)");
    properties.add("key", keyProp);

    JsonObject valueProp = new JsonObject();
    valueProp.addProperty(
        "description", "The new value for the setting (boolean, integer, or string)");
  ...
  public JsonObject execute(JsonObject arguments) {
    ...
    if (!SettingsAdapter.isKnownSetting(key)) {
      ...
      allowed.add("ICEBRIDGE_ENABLED");
      allowed.add("ICEBRIDGE_USE_REMOTE");
      allowed.add("ICEBRIDGE_REMOTE_URL");
