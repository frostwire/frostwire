...
public class VPNDropProtectionTool implements MCPTool {
  ...
  public JsonObject execute(JsonObject arguments) {
    ...
    postStatusRefresh(VPNDropProtectionTool::refreshStatusLine);
  ...
  static void postStatusRefresh(Runnable action) {
    GUIMediator.safeInvokeLater(action);
  }

  private static void refreshStatusLine() {
    try {
      GUIMediator mediator = GUIMediator.instance();
      if (mediator == null || mediator.getStatusLine() == null) {
        return;
      }
