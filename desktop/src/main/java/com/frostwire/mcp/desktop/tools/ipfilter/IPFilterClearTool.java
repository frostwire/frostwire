...
public class IPFilterClearTool implements MCPTool {
  ...
  public JsonObject execute(JsonObject arguments) {
    ...
    try {
      IPFilterTableAccess.clear();
    } catch (RuntimeException e) {
      LOG.error("IPFilterClearTool: Failed to clear IP filter table", e);
      result.addProperty("error", "Failed to clear IP filter table: " + e.getMessage());
      result.addProperty("cleared", false);
      return result;
    }
