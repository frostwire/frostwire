...
public class DownloadTorrentTool implements MCPTool {
  ...
  public JsonObject execute(JsonObject arguments) {
    ...
    File saveDir =
        arguments.has("saveDir")
            ? new File(arguments.get("saveDir").getAsString())
              ...
              () -> {
                try {
                  switch (source.toLowerCase()) {
                    ...
                    default:
                      return TransferAdapter.errorJson(
                          "Invalid source type: " + source + ". Use magnet, url, or file.");
                  }
                } catch (Exception e) {
  ...
  private JsonObject downloadMagnet(String magnetUri, File saveDir) {
    try {
      ...
      BTEngine.getInstance()
          .download(ti, saveDir, null, LibTorrentMagnetDownloader.parsePeers(magnetUri), true);
      String infoHash =
          ti.infoHashV1() != null
              ? ti.infoHashV1().toString().toLowerCase()
              : ti.infoHashV2().toString().toLowerCase();
