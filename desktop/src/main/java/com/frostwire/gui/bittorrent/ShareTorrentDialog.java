...
public class ShareTorrentDialog extends JDialog {
  ...
  private void initURLShortenerListeners() {
    final URLShortenerHttpClientListener tinyurlShortenerListener =
        new URLShortenerHttpClientListener("http://tinyurl.com/api-create.php?url=" + getLink());
    // this one is different because google got fancy with POST and Json.
    final URLShortenerHttpClientListener googShortenerListener =
        new GoogleURLShortenerListener(
            "https://www.googleapis.com/urlshortener/v1/url?key=AIzaSyDw6xPSKYZyOIv7rq2A0R9fDvzsrpI25I0");
    // we'll use one at random to not exhaust the monthly quotas.
    shortenerListeners =
        new LinkedList<>(Arrays.asList(googShortenerListener, tinyurlShortenerListener));
  }
    ...
    GUIMediator.safeInvokeLater(
        () -> {
          textArea.setText(
              I18n.tr("Download")
                  + " \""
                  + torrent_name.replace("_", " ")
                  + "\" "
                  + I18n.tr("at")
                  + " "
                  + getLink().trim()
                  + " "
                  + I18n.tr("via FrostWire"));
          textArea.selectAll();
        });
  }
  ...
  private void setupUI() {
    ...
    JLabel _introLabel =
        new JLabel(
            folderTorrent
                ? String.format(
                    I18n.tr("Use the following text to share the \"%s\" folder"), torrent_name)
                : String.format(
                    I18n.tr("Use the following text to share the \"%s\" file"), torrent_name));
          ...
          public void mouseClicked(MouseEvent e) {
            boolean allTextAlreadySelected =
                textArea.getSelectedText() != null
                    && textArea.getSelectedText().equals(textArea.getText());
    ...
    performAsyncURLShortening(
        shortenerListeners.get(new Random().nextInt(shortenerListeners.size() - 1)));
    // BUTTON ROW
    initActions();
    c = new GridBagConstraints();
    ...
    ButtonRow buttonRow = new ButtonRow(actions, ButtonRow.X_AXIS, ButtonRow.RIGHT_GLUE);
    ...
    JLabel _tipsLabel =
        new JLabel(
            "<html><p> > "
                + I18n.tr(
                    "<strong>Keep FrostWire Open</strong> until the file has been downloaded by at least one other friend.")
                + "</p><p>&nbsp;</p><p> >"
                + I18n.tr(
                    "<strong>The more, the merrier.</strong> The more people sharing the faster it can be downloaded by others.")
                + "</p><p>&nbsp;</p><p> >"
                + I18n.tr(
                    "<strong>Your files can be discovered by others.</strong> Once you share this link and you seed the files they will be available to everybody on the BitTorrent network.")
                + "</p></html>");
    // Defer font loading to avoid EDT violation
  ...
  /**
   * Animates the feedbackLabel (which lives on the Glass Pane invisibly and is shown during the
   * animation)
   *
   * <p>To animate this component we wrap the JLabel on a TimelineJLabel. If we ever need to animate
   * more labels, we'll probably just extend JLabel in the future.
   *
   * <p>The animation interpolates 2 properties. The color of the text, and the y position of the
   * text.
   *
  ...
  private class URLShortenerHttpClientListener extends HttpClientListenerAdapter {
        ...
        try {
          ...
          final String jsonResponse =
              browser.post(
                  ...
                  "application/json",
                  false);
          GoogleURLShortenerResponse response =
              JsonUtils.toObject(jsonResponse, GoogleURLShortenerResponse.class);
          link = response.id;
          updateTextArea(); // happens safely on UI thread.
        } catch (Throwable ignored) {
        }
  ...
  private class PostToXAction extends AbstractAction {
    PostToXAction() {
      putValue(Action.NAME, I18n.tr("Post it on X"));
      putValue(Action.SHORT_DESCRIPTION, I18n.tr("Post the message above on X"));
      putValue(LimeAction.ICON_NAME, "X");
    }

    @Override
    public void actionPerformed(ActionEvent e) {
      GUIMediator.openURL("https://x.com/intent/post?text=" + UrlUtils.encode(textArea.getText()));
    }
  }
  ...
  private class CopyToClipboardAction extends AbstractAction {
    ...
    public void actionPerformed(ActionEvent e) {
      ...
      showFeedback(
          getTitle(), source.getLocationOnScreen().getX(), source.getLocationOnScreen().getY());
    }
  }
  ...
  private class CopyMagnetAction extends AbstractAction {
    ...
    @Override
    public void actionPerformed(ActionEvent e) {
      GUIMediator.setClipboardContent(
          TorrentUtil.getMagnet(info_hash) + BTEngine.getInstance().magnetPeers());
