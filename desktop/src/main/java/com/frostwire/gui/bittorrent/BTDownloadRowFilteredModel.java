...
public class BTDownloadRowFilteredModel extends BTDownloadModel {
  ...
  /**
   * Cache hidden uploads count to avoid iterating HIDDEN list on every status refresh. The cache
   * update is done in background thread to prevent EDT blocking.
   */
  ...
  BTDownloadRowFilteredModel(TableLineFilter<BTDownloadDataLine> f) {
    if (f == null) throw new NullPointerException("null filter");
    FILTER = f;
  ...
  /** Snapshot both visible and filtered-out transfers without touching libtorrent. EDT only. */
  public List<BTDownload> snapshotAllDownloads() {
    if (!SwingUtilities.isEventDispatchThread()) {
      throw new IllegalStateException("Transfer table snapshot must run on the EDT");
    }
    List<BTDownload> downloads = new ArrayList<>(_list.size() + HIDDEN.size());
    for (BTDownloadDataLine line : _list) {
      downloads.add(line.getInitializeObject());
    }
    for (BTDownloadDataLine line : HIDDEN) {
      downloads.add(line.getInitializeObject());
    }
    return downloads;
  }
  ...
  /**
   * Schedules a background thread to update the hidden uploads cache. This prevents EDT blocking
   * when there are many hidden downloads.
   */
          ...
          try {
            ...
            try {
              for (BTDownloadDataLine aHIDDEN : HIDDEN) {
                ...
                if (downloader != null
                    && downloader.isCompleted()
                    && downloader.getState() == TransferState.SEEDING) {
                  count++;
                }
              }
            } catch (Throwable e) {
