...
/**
 * This class acts as a mediator between all the components of the download window. It also
 * constructs all the download window components.
 *
 * @author gubatron
 * @author aldenml
 */
public final class BTDownloadMediator
  ...
  public void setBTDownloadSelectionListener(
      BTDownloadSelectionListener transferSelectionListener) {
  ...
  /**
   * Override the default refreshing so that we can set the clear button appropriately. Now
   * delegates expensive state checks to background thread.
   */
   ...
   *
   * <p>Adds a new Downloads to the list of Downloads, obtaining the necessary information from the
   * supplied `Downloader`.
   *
   * <p>If the download is not already in the list, then it is added.
   */
   ...
   *
   * <p>Takes action upon downloaded theme files, asking if the user wants to apply the theme.
   *
   * <p>Removes a download from the list if the user has configured their system to automatically
   * clear completed download and if the download is complete.
   *
   * @param downloader the `Downloader` to remove from the list if it is complete.
   */
            ...
            () -> {
              ...
              try {
                if (dl instanceof SoundcloudDownload) {
                  ...
                  if (saveLocation.isFile()) {
                    ...
                    hasMP4s =
                        org.limewire.util.FileUtils.hasExtension(
                            saveLocation.getAbsolutePath(), "mp4");
              ...
              GUIMediator.safeInvokeLater(
                  () -> {
                    ...
                    playSingleMediaFileAction.setEnabled(
                        getSelectedDownloaders().length == 1 && fHasMediaFiles && fIsSingleFile);
                  });
            });
  }

  /**
   * Handles the deselection of all rows in the download table, disabling all necessary buttons and
   * menu items.
   */
  ...
  /**
   * Handles the selection of the specified row in the download window, enabling or disabling
   * buttons and chat menu items depending on the values in the row.
   *
  ...
  public void openTorrentFileForSeed(final File torrentFile, final File saveDir) {
    ...
    if (VPNDropGuard.canUseBitTorrent()) {
      try {
        ...
        try {
          // Ensure the .torrent exists under the configured Torrents directory so it can be
          // discovered/restored
          File torrentsDir =
              com.limegroup.gnutella.settings.SharingSettings.TORRENTS_DIR_SETTING.getValue();
          if (torrentsDir != null) {
        ...
        GUIMediator.safeInvokeLater(
            () -> {
              LOG.error(
                  "openTorrentFileForSeed(): BTEngine.getInstance().download(torrentFile, saveDir, null) failed: ",
                  e);
              if (!e.toString().contains("No files selected by user")) {
                // could not read torrent file or bad torrent file.
                GUIMediator.showError(
                    I18n.tr(
                        "FrostWire was unable to load the torrent file \"{0}\", - it may be malformed or FrostWire does not have permission to access this file.",
                        torrentFile.getName()),
  ...
  protected void setDefaultRenderers() {
    ...
    TABLE.setDefaultRenderer(
        TransferDetailFiles.TransferItemHolder.class, new TransferDetailFilesActionsRenderer());
  }

  @Override
  protected void setDefaultEditors() {
    BTDownloadDataLine.PAYMENT_OPTIONS_COLUMN.setCellEditor(
        new GenericCellEditor(getPaymentOptionsRenderer()));
    BTDownloadDataLine.ACTIONS_COLUMN.setCellEditor(
        new GenericCellEditor(getTransferActionsRenderer()));
  ...
  public void openTorrentFile(
      ...
      final boolean partialDownload,
      final Runnable onOpenRunnableForUIThread) {
    if (VPNDropGuard.canUseBitTorrent()) {
      SwingUtilities.invokeLater(
          () -> {
            try {
              ...
              if (partialDownload) {
                PartialFilesDialog dlg =
                    new PartialFilesDialog(GUIMediator.getAppFrame(), torrentFile);
                dlg.setVisible(true);
  ...
  private void restoreSorting() {
    if (EventQueue.isDispatchThread()) {
      throw new RuntimeException(
          "BTDownloadMediator.restoreSorting() cannot be called from the EDT");
    }
    ...
    if (sortIndex != -1
        && column != null
        && TablesHandlerSettings.getVisibility(column.getId(), column.getDefaultVisibility())
            .getValue()) {
  ...
  public void downloadSoundcloudFromTrackUrlOrSearchResult(
      final String trackUrl, final SoundcloudSearchResult sr, boolean fromPastedUrl) {
    if (sr != null) {
      DesktopParallelExecutor.execute(
          () -> {
            System.out.println(
                "BTDownloadMediator.downloadSoundcloudFromTrackUrlOrSearchResult about to get download url");
            final String downloadUrl = sr.getDownloadUrl();
            System.out.println(
                "BTDownloadMediator.downloadSoundcloudFromTrackUrlOrSearchResult: " + downloadUrl);
            GUIMediator.safeInvokeLater(
                () -> {
      ...
      DesktopParallelExecutor.execute(
          () -> {
            try {
              ...
              LinkedList<SoundcloudSearchResult> results =
                  SoundcloudUtils.fromJson(json, fromPastedUrl);
              if (results.isEmpty()) {
                GUIMediator.safeInvokeLater(
                    () ->
                        GUIMediator.showError(
                            I18n.tr("Sorry, Couldn't find a valid download location at")
                                + "<br><br><a href=\""
                                + trackUrl
                                + "\">"
                                + trackUrl
  ...
  public void openHttp(
    ...
    openHttp(httpUrl, title, saveFileAs, fileSize, extractAudio, null);
  }
          ...
          final HttpDownload downloader =
              new HttpDownload(
                ...
                protected void onComplete() {
                  ...
                  if (savedFile.exists()) {
                    // if extract audio and delete original, we need to do this before the file is
                    // scanned
                    if (extractAudio) {
                            ...
                            () -> {
                              ...
                              BackgroundQueuedExecutorService.schedule(
                                  () -> {
                                    ...
                                    GUIMediator.safeInvokeLater(
                                        () ->
                                            LibraryFilesTableMediator.instance().setSelectedRow(0));
                                  });
                            });
                      }
                    }
  ...
  private File extractAudio(File videoMp4) {
    ...
    File extractedAudio =
        new File(mp4.getParentFile(), FilenameUtils.getBaseName(mp4.getName()) + ".m4a")
            .getAbsoluteFile();
    ...
    try {
      ...
      LOG.error(
          "BTDownloadMediator.extractAudioAndRemoveOriginalVideo() error extracting audio from mp4 file: "
              + mp4.getAbsolutePath(),
          e);
      return null;
    }
