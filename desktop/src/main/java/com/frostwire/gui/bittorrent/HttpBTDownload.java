...
abstract class HttpBTDownload implements BTDownload {
  ...
  HttpBTDownload(String filename, long size) {
    completeFile =
        org.limewire.util.FileUtils.buildFile(
            SharingSettings.TORRENT_DATA_DIR_SETTING.getValue(), filename);
    this.size = size;
    dateCreated = new Date();
    bytesReceived = 0;
    httpClient = HttpClientFactory.newInstance(HttpClientFactory.HttpContext.DOWNLOAD);
  ...
  static File getIncompleteFolder() {
    ...
    if (!incompleteFolder.exists()) {
      if (!incompleteFolder.mkdirs()) {
        LOG.warn(
            "BTDownloadUtils.getIncompleteFolder(): could not mkdirs for ["
  ...
  void updateAverageDownloadSpeed() {
    ...
    if (isCompleted()) {
      ...
      averageSpeed =
          ((bytesReceived - totalReceivedSinceLastSpeedStamp) * 1000) / (now - speedMarkTimestamp);
      speedMarkTimestamp = now;
  ...
  public void remove() {
    ...
    if (deleteDataWhenRemoved) {
      ...
      if (!getSaveLocation().delete()) {
        LOG.warn(
            "HttpBTDownload.remove(): could not delete ["
                + getSaveLocation().getAbsolutePath()
                + "]");
      }
    }
  }
