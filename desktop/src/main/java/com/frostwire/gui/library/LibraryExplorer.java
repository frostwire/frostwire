...
import com.limegroup.gnutella.gui.util.CoalescingGate;
...
public class LibraryExplorer extends AbstractLibraryListPanel {
  ...
  /**
   * Collapses a burst of media-type rescan requests (e.g. one per completed download) into a single
   * running scan plus one follow-up. Otherwise the bounded DesktopParallelExecutor (4 threads + 4
   * queued) overflows and rejects the excess tasks.
   */
  private final CoalescingGate mediaTypeSearchGate = new CoalescingGate();

  private volatile MediaTypeSavedFilesDirectoryHolder pendingMediaTypeHolder;
  ...
  public void refreshSelection(boolean clearCache) {
    ...
    // MEDIA TYPES
      ...
      MediaTypeSavedFilesDirectoryHolder mtsfdh =
          (MediaTypeSavedFilesDirectoryHolder) directoryHolder;
      if (clearCache) {
        mtsfdh.clearCache();
      }
      LibraryMediator.instance().updateTableFiles(directoryHolder);
      requestMediaTypeSearch(mtsfdh);
    }
  ...
  private void setupModel() {
    ...
    root.add(
        new DirectoryHolderNode(
            new SavedFilesDirectoryHolder(
                SharingSettings.TORRENT_DATA_DIR_SETTING, I18n.tr("Default Save Folder"))));
  ...
  DirectoryHolder getSelectedDirectoryHolder() {
    ...
    LibraryNode node = (LibraryNode) tree.getLastSelectedPathComponent();
    return node instanceof DirectoryHolderNode
        ? ((DirectoryHolderNode) node).getDirectoryHolder()
        : null;
  }
  ...
  /**
   * Schedules a media-type scan through the coalescing gate so a burst of requests runs at most one
   * scan plus one follow-up, never overloading {@link DesktopParallelExecutor}.
   */
  private void requestMediaTypeSearch(MediaTypeSavedFilesDirectoryHolder holder) {
    if (holder == null) {
      return;
    }
    pendingMediaTypeHolder = holder;
    if (!mediaTypeSearchGate.begin()) {
      return;
    }
    submitMediaTypeSearch();
  }

  private void submitMediaTypeSearch() {
    MediaTypeSavedFilesDirectoryHolder holder = pendingMediaTypeHolder;
    if (holder == null) {
      mediaTypeSearchGate.finish();
      return;
    }
    try {
      DesktopParallelExecutor.execute(
          () -> {
            try {
              new SearchByMediaTypeRunnable(holder).run();
            } finally {
              if (mediaTypeSearchGate.finish()) {
                submitMediaTypeSearch();
              }
            }
          });
    } catch (RejectedExecutionException rejected) {
      // Executor saturated: release the gate so a later request can retry.
      mediaTypeSearchGate.finish();
    }
  }
  ...
  private void selectMediaTypeSavedFilesDirectoryHolderbyType(MediaType mediaType) {
    try {
      ...
      if (selectedValue instanceof DirectoryHolderNode
          && ((DirectoryHolderNode) selectedValue).getDirectoryHolder()
              instanceof MediaTypeSavedFilesDirectoryHolder
          && ((MediaTypeSavedFilesDirectoryHolder)
                  ((DirectoryHolderNode) selectedValue).getDirectoryHolder())
              .getMediaType()
              .equals(mediaType)) {
        // already selected
      ...
      while (e.hasMoreElements()) {
        ...
        if (node instanceof DirectoryHolderNode) {
          ...
          if (holder instanceof MediaTypeSavedFilesDirectoryHolder
              && ((MediaTypeSavedFilesDirectoryHolder) holder).getMediaType().equals(mediaType)) {
  ...
  private boolean selectionListenerForSameItem(Class<?> clazz) {
    LibraryNode selectedValue = (LibraryNode) tree.getLastSelectedPathComponent();
    if (selectedValue instanceof DirectoryHolderNode
        && clazz.isInstance(((DirectoryHolderNode) selectedValue).getDirectoryHolder())) {
      // already selected
      try {
        treeSelectionListener.valueChanged(null);
      } catch (Exception e) {
    ...
    public void run() {
      try {
        ...
        for (File dir : directories) {
          ...
          } else if (!dir.equals(LibrarySettings.USER_MUSIC_FOLDER.getValue())
