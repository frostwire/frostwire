...
public final class TransferDetailFiles extends JPanel
  ...
  /**
   * Cached full holder list (includes skipped files) for the current torrent. Rebuilt from JNI only
   * when the transfer data changes (new pieces, priority changes). Checkbox clicks merely filter
   * this cached list on the EDT — zero JNI, instant response.
   */
  ...
  /**
   * Monotonically-increasing sequence number for rebuildHolders calls. Each invokeLater checks
   * whether a newer rebuild has superseded it, preventing stale updates from out-of-order
   * background completions.
   */
  private volatile long rebuildSequence = 0;

  /**
   * Torrent progress/state captured when {@link #allHolders} was last built. The refresh loop
   * compares these to detect drift while a transfer is still moving; a settled (all-complete,
   * unchanged) torrent skips JNI.
   */
  private int lastBuiltTorrentProgress = -1;

  private TransferState lastBuiltTorrentState;
  private volatile long lastProgressRebuildMillis;

  TransferDetailFiles() {
    tableMediator = new TransferDetailFilesTableMediator();
    showSkippedCheckbox = new JCheckBox(I18n.tr("Show skipped files"));
    showSkippedCheckbox.setSelected(
        BittorrentSettings.SHOW_SKIPPED_FILES_IN_TRANSFER_DETAIL.getValue());
    showSkippedCheckbox.setVisible(false);
    showSkippedCheckbox.addItemListener(
        e -> {
          BittorrentSettings.SHOW_SKIPPED_FILES_IN_TRANSFER_DETAIL.setValue(
              showSkippedCheckbox.isSelected());
  ...
  /**
   * Filters the cached {@link #allHolders} based on the current "Show skipped files" setting and
   * updates the table — entirely on the EDT, zero JNI calls.
   */
  ...
  public void updateData(BittorrentDownload btDownload) {
    ...
    // priority-changed / download-started callbacks), unless torrent
    // progress/state drifted while the transfer is still moving.
    if (this.btDownload == btDownload && !allHolders.isEmpty()) {
      refreshStaleProgress(btDownload);
      return;
    }
  ...
  /**
   * Refresh-loop entry: re-applies the skipped-files filter, rebuilding the cached holders first
   * when torrent progress/state drifted while the transfer is still moving. Settled torrents (all
   * files complete, progress and state unchanged) skip JNI entirely.
   */
  private void refreshStaleProgress(BittorrentDownload target) {
    if (target.getDl() == null) {
      SwingUtilities.invokeLater(this::applyFilterOnEdt);
      return;
    }
    if (!SwingUtilities.isEventDispatchThread() && holdersSettled(target)) {
      SwingUtilities.invokeLater(this::applyFilterOnEdt);
      return;
    }
    long now = System.currentTimeMillis();
    if (now - lastProgressRebuildMillis < 2500) {
      SwingUtilities.invokeLater(this::applyFilterOnEdt);
      return;
    }
    lastProgressRebuildMillis = now;
    rebuildHolders(target);
  }

  private boolean holdersSettled(BittorrentDownload target) {
    for (TransferItemHolder holder : allHolders) {
      if (!holder.complete) {
        return false;
      }
    }
    try {
      com.frostwire.bittorrent.BTDownload download = target.getDl();
      return download != null
          && download.getProgress() == lastBuiltTorrentProgress
          && download.getState() == lastBuiltTorrentState;
    ...
    Runnable task =
        () -> {
          try {
            ...
            int builtProgress = -1;
            TransferState builtState = null;
            try {
              builtProgress = target.getDl().getProgress();
              builtState = target.getDl().getState();
            } catch (Throwable ignored) {
            }
            final int finalBuiltProgress = builtProgress;
            final TransferState finalBuiltState = builtState;
            SwingUtilities.invokeLater(
                () -> {
                  ...
                  this.lastBuiltTorrentProgress = finalBuiltProgress;
                  this.lastBuiltTorrentState = finalBuiltState;
  ...
  /**
   * A Transfer item does not know its own position within the torrent item list, this class keeps
   * that number and a reference to the transfer item.
   *
   * <p>Also, this is necessary to update the table, since tableMediator.update() doesn't work with
   * plain TransferItems
   */
  public static class TransferItemHolder {
    ...
    TransferItemHolder(int fileOffset, TransferItem transferItem) {
      ...
      this.fileType =
          extensionSeparator >= 0 && extensionSeparator + 1 < fileName.length()
    ...
    public boolean equals(Object obj) {
      return obj instanceof TransferItemHolder
          && ((TransferItemHolder) obj).fileOffset == fileOffset;
    }
  }
}
