...
public final class TransferDetailGeneral extends JPanel
  ...
  TransferDetailGeneral() {
    ...
    Color backgroundColor =
        ThemeMediator.isLightThemeOn() ? defaultBG : ThemeMediator.APP_REALLY_DARK_COLOR;
    Color foregroundColor =
        ThemeMediator.isLightThemeOn() ? defaultFG : ThemeMediator.APP_REALLY_DARK_COLOR;
    ...
    midPanel.add(
        ...
        "gapright 50, wrap");
    sequentialDownloadCheckbox.setToolTipText(
        I18n.tr("Download pieces in order (helps previewing media)"));
    sequentialDownloadCheckbox.addActionListener(e -> onSequentialDownloadToggled());
    // 3rd Section, "GENERAL"
    // Save location
    // InfoHash
    // Magnet URL
    ...
    saveLocationGrayLabel =
        new JGrayLabel("<html><a href=\"#\">" + I18n.tr("Save location") + "</a>:</html>");
    ...
    JPanel lowerPortionWhiteBGContainer =
        new JPanel(new MigLayout("insets 0 0 0 0, gap 0 0, fill"));
    lowerPortionWhiteBGContainer.setOpaque(true);
    // Empty border for margins, and line border for visual delimiter
    lowerPortionWhiteBGContainer.setBorder(
  ...
  public void updateData(BittorrentDownload guiBtDownload) {
    ...
    GUIMediator.safeInvokeLater(
        () -> {
          ...
          saveLocationGrayLabel.addMouseListener(
              new MouseAdapter() {
                @Override
                public void mouseClicked(MouseEvent e) {
                  GUIMediator.launchExplorer(
                      new File(
                          finalGuiBtDownload.getDl().getSavePath(), finalGuiBtDownload.getName()));
                }
              });
  ...
  private void onSequentialDownloadToggled() {
    BTDownload target = btDownload;
    if (target == null) {
      return;
    }
  ...
  /**
   * Converts a value in seconds to: "d:hh:mm:ss" where d=days, hh=hours, mm=minutes, ss=seconds, or
   * "h:mm:ss" where h=hours<24, mm=minutes, ss=seconds, or "m:ss" where m=minutes<60, ss=seconds
   */
