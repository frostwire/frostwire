...
public abstract class SearchEngine {
  ...
  private static final UserAgent FROSTWIRE_ANDROID_USER_AGENT =
      new UserAgent(
          getOSVersionString(), Constants.FROSTWIRE_VERSION_STRING, Constants.FROSTWIRE_BUILD);
  ...
  public TellurideCourier.SearchPerformer getTelluridePerformer(
      long currentSearchToken, String pageUrl, SearchResultListAdapter adapter) {
    // override me
    return null;
  }
  ...
  /**
   * This will include all engines, even if they have been marked as inactive via remote config or
   * by some other mean.
   *
   * <p>It will only exclude non-ready engines if excludeNonReady=true
   *
   * <p>We need to include inactive engines so that places like the SearchEnginesPreferenceFragment
   * can separate active engines from inactive engines and do things like hide them from the user
   * interface in the case of FrostWire Basic where some engines are not allowed in google play
   */
  ...
  public static final SearchEngine SOUNCLOUD =
      new SearchEngine("Soundcloud", Constants.PREF_KEY_SEARCH_USE_SOUNDCLOUD) {
        @Override
  ...
  public static final SearchEngine ARCHIVE =
      new SearchEngine("Archive.org", Constants.PREF_KEY_SEARCH_USE_ARCHIVEORG) {
        @Override
        public ISearchPerformer getPerformer(long token, String keywords) {
  ...
  public static final SearchEngine FROSTCLICK =
      new SearchEngine("FrostClick", Constants.PREF_KEY_SEARCH_USE_FROSTCLICK) {
        @Override
        public ISearchPerformer getPerformer(long token, String keywords) {
          ...
          return SearchPerformerFactory.createSearchPerformer(
              ...
              new FrostClickSearchPattern(
                  FROSTWIRE_ANDROID_USER_AGENT.toString(),
                  FROSTWIRE_ANDROID_USER_AGENT.getUUID(),
                  FROSTWIRE_ANDROID_USER_AGENT.getHeadersMap()),
              null, // No crawling needed
              DEFAULT_TIMEOUT);
        }
      };
  ...
  public static final SearchEngine NYAA =
      new SearchEngine("Nyaa", Constants.PREF_KEY_SEARCH_USE_NYAA) {
        @Override
        public ISearchPerformer getPerformer(long token, String keywords) {
  ...
  public static final SearchEngine TPB =
      new SearchEngine("TPB", Constants.PREF_KEY_SEARCH_USE_TPB) {
        ...
        public ISearchPerformer getPerformer(long token, String keywords) {
          if (domainName == null) {
            throw new RuntimeException(
                "check your logic, this search performer has no domain name ready");
          }
                  ...
                  () -> {
                    ...
                    domainName =
                        UrlUtils.getFastestMirrorDomain(
                            httpClient, TPBMirrors.getMirrors(), 6000, 6);
                  })
              .start();
        }
  ...
  public static final SearchEngine ONE337X =
      new SearchEngine("1337x", Constants.PREF_KEY_SEARCH_USE_ONE337X) {
        @Override
        public ISearchPerformer getPerformer(long token, String keywords) {
          // V2: Using new flat architecture with crawling strategy
  ...
  public static final SearchEngine TORRENTZ2 =
      new SearchEngine("torrentz2", Constants.PREF_KEY_SEARCH_USE_TORRENTZ2) {
  ...
  public static final SearchEngine MAGNETDL =
      new SearchEngine("magnetdl", Constants.PREF_KEY_SEARCH_USE_MAGNETDL) {
  ...
  public static final SearchEngine TORRENTSCSV =
      new SearchEngine("TorrentsCSV", Constants.PREF_KEY_SEARCH_USE_TORRENTSCSV) {
        @Override
        public ISearchPerformer getPerformer(long token, String keywords) {
  ...
  public static final SearchEngine KNABEN =
      new SearchEngine("Knaben", Constants.PREF_KEY_SEARCH_USE_KNABEN) {
        @Override
        public ISearchPerformer getPerformer(long token, String keywords) {
  ...
  public static final SearchEngine BITSEARCH =
      new SearchEngine("Bitsearch", Constants.PREF_KEY_SEARCH_USE_BITSEARCH) {
        @Override
        public ISearchPerformer getPerformer(long token, String keywords) {
  ...
  public static final SearchEngine TELLURIDE_COURIER =
      new SearchEngine("Telluride Courier", Constants.PREF_KEY_SEARCH_USE_TELLURIDE_COURIER) {
        @Override
        public ISearchPerformer getPerformer(long token, String keywords) {
          return null;
        }
        ...
        @Override
        public TellurideCourier.SearchPerformer<SearchResultListAdapter> getTelluridePerformer(
            long token, String pageUrl, SearchResultListAdapter adapter) {
          return new TellurideCourier.SearchPerformer<>(token, pageUrl, adapter);
        }
      };

  public static final SearchEngine YT =
      new SearchEngine("YT", Constants.PREF_KEY_SEARCH_USE_YT) {
        @Override
        public ISearchPerformer getPerformer(long token, String keywords) {
  ...
  /**
   * Diagnostic LocalIndex search (same performer as desktop LOCAL). Same index {@link
   * RelaySearchService} uses for inbound IceBridge answers — not a peer search. Ready once {@link
   * #LOCAL_WIRING} has a {@link LocalIndex}.
   */
  public static final SearchEngine LOCAL =
      new SearchEngine("Local (test)", Constants.PREF_KEY_SEARCH_USE_LOCAL) {
        @Override
        public ISearchPerformer getPerformer(long token, String keywords) {
          if (!isReady()) {
            return null;
          ...
          return new LocalSharedTorrentSearchPerformer(
              token, keywords, LOCAL_WIRING.localIndex(), LOCAL_WIRING.karmaCache());
        }
  ...
  public static final SearchEngine DISTRIBUTED =
      new SearchEngine("Distributed", Constants.PREF_KEY_SEARCH_USE_DISTRIBUTED) {
        @Override
        public ISearchPerformer getPerformer(long token, String keywords) {
          if (!isReady()) {
            return null;
