...
public class MainActivity extends AbstractActivity
  ...
  protected void initComponents(Bundle savedInstanceState) {
    ...
    if (playerSubscription != null) {
      ...
      TimerService.reSubscribe(
          miniPlayerView.getRefresher(),
          playerSubscription,
          MiniPlayerView.REFRESHER_INTERVAL_IN_SECS);
    } else {
      playerSubscription =
          TimerService.subscribe(
              miniPlayerView.getRefresher(), MiniPlayerView.REFRESHER_INTERVAL_IN_SECS);
    }
  ...
  protected void onNewIntent(Intent intent) {
    ...
    if (intent.hasExtra(Constants.EXTRA_DOWNLOAD_COMPLETE_NOTIFICATION)) {
      SystemUtils.postToHandler(
          SystemUtils.HandlerThreadName.MISC, () -> onDownloadCompleteNotification(intent));
    }
  ...
  private static boolean isMusicPlayerIntent(Intent intent) {
    return intent != null
        && (ACTION_MUSIC_PLAYER.equals(intent.getAction())
      ...
      if (uri != null) {
        if (uri.startsWith("file")
          ...
          TransferManager.instance()
              .downloadTorrent(uri, new HandpickedTorrentDownloadDialogOnFetch(this, false));
              ...
              () -> {
                ...
                if (newUri != null) {
                  TransferManager.instance()
                      .downloadTorrent(
                          newUri,
                          new HandpickedTorrentDownloadDialogOnFetch(MainActivity.this, false));
                }
              });
        }
      } else {
  ...
  private SparseArray<DangerousPermissionsChecker<MainActivity>> initPermissionsCheckers() {
    ...
    final DangerousPermissionsChecker<MainActivity> externalStorageChecker =
        new DangerousPermissionsChecker<>(
            this, DangerousPermissionsChecker.EXTERNAL_STORAGE_PERMISSIONS_REQUEST_CODE);
    checkers.put(
    ...
    final DangerousPermissionsChecker<MainActivity> postNotificationsChecker =
        new DangerousPermissionsChecker<>(
            this, DangerousPermissionsChecker.POST_NOTIFICATIONS_PERMISSIONS_REQUEST_CODE);
    checkers.put(
        DangerousPermissionsChecker.POST_NOTIFICATIONS_PERMISSIONS_REQUEST_CODE,
        postNotificationsChecker);
    return checkers;
  }
  ...
  /**
   * Launches a promo video preview intent via the Activity Result API. Called by {@link
   * com.frostwire.android.gui.adapters.PromotionsAdapter}.
   */
  ...
  private void checkExternalStoragePermissions() {
    DangerousPermissionsChecker<MainActivity> checker =
        permissionsCheckers.get(
            DangerousPermissionsChecker.EXTERNAL_STORAGE_PERMISSIONS_REQUEST_CODE);
    if (!externalStoragePermissionsRequested
        && checker != null
        && checker.noExternalStorageAccess()
        && !checker.shouldShowExternalStorageAccessRationale()) {
  ...
  private void mainResume() {
    ...
    if (firstTime) {
      if (ConfigurationManager.instance()
        ...
        UIUtils.showDismissableMessage(
            findView(R.id.activity_main_parent_layout), R.string.cannot_start_engine_without_vpn);
      } else {
        firstTime = false;
  ...
  private void checkLastSeenVersionBuild() {
    ...
    final String lastSeenVersionBuild =
        CM.getString(Constants.PREF_KEY_CORE_LAST_SEEN_VERSION_BUILD);
    final String currentVersionBuild =
        Constants.FROSTWIRE_VERSION_STRING + "." + Constants.FROSTWIRE_BUILD;
  ...
  private void setupInitialFragment(Bundle savedInstanceState) {
    ...
    if (savedInstanceState != null) {
      try {
        fragment =
            getSupportFragmentManager().getFragment(savedInstanceState, CURRENT_FRAGMENT_KEY);
    ...
    public void onReceive(Context context, Intent intent) {
      ...
      if (Constants.ACTION_NOTIFY_DATA_INTERNET_CONNECTION.equals(action)) {
        ...
        if (!isDataUp && !!MainActivity.this.isPaused() && !MainActivity.this.isFinishing()) {
          UIUtils.showDismissableMessage(
              findView(android.R.id.content), R.string.no_data_check_internet_connection);
        }
